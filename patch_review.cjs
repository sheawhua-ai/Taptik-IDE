...
if (match) {
    ...
    // Also remove any remaining occurrences of '共 {filteredTasks.length} 项' 
    const bottomRegex = /<div className="flex items-center justify-between mt-1">[\s\S]*?<\/div>\s*<\/div>/g;
    code = code.replace(bottomRegex, '');
