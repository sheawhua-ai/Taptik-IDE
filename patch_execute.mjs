...
const regex = /const handleExecute = \([\s\S]*?\n  const handleToggleProgress/m;

const newHandleExecute = `const handleExecute = (customQuery?: string) => {
    ...
    // Default to pushing confirmation card for ANY request after onboarding
    setTimeout(() => {
      setMessages(prev => prev.map(m => m.id === agentMsgId ? {
        ...m,
        isThinking: false,
        card: {
          ...
          tools: ['策略专家搜索分析', '内容专家生成图文', '调用品牌知识库'],
          destinations: ['当前对话', '对应项目库', '内容车间草稿区'],
          wontDo: ['自动发布', '自动通知客户', '自动修改排期'],
...
content = content.replace(regex, newHandleExecute);
