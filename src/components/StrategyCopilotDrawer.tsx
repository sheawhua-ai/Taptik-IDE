  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <motion.div
        ...
        className="w-[800px] bg-page-bg h-full shadow-2xl flex flex-col relative z-10"
      ...
      >
        <div className="h-16 border-b border-border-default flex items-center justify-between px-6 shrink-0 bg-surface-1">
          <div className="flex items-center gap-2">
            <Bot size={20} className="text-brand-logo" />
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Chat Area */}
          <div className="flex-1 flex flex-col bg-surface-1 border-r border-border-default relative">
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-xl p-4 text-[14px] leading-relaxed shadow-sm ${
                    m.role === 'user' 
                      ? 'bg-btn-main text-white rounded-br-none' 
              ...
              {isThinking && (
                <div className="flex justify-start">
                  <div className="bg-brand-light/80 border border-primary-100/50 text-primary-900 rounded-xl rounded-bl-none p-4 shadow-sm flex items-center gap-2">
            ...
            <div className="p-4 bg-surface-1 border-t border-border-default">
              <div className="relative flex items-end bg-page-bg border border-border-default rounded-xl overflow-hidden focus-within:border-primary-400 focus-within:ring-1 focus-within:ring-primary-400 transition-all shadow-inner">
                <textarea
                  value={input}
                  ...
                  className="w-full bg-transparent p-4 text-[14px] text-text-main resize-none outline-none max-h-[120px] min-h-[60px]"
                />
                <button
                  onClick={handleSend}
          ...
          {/* Slots / Context Area */}
          <div className="w-[280px] bg-page-bg flex flex-col shrink-0">
            <div className="p-5 border-b border-border-default bg-hover-bg/50">
            ...
            <div className="flex-1 p-5 space-y-4 overflow-y-auto">
              <div className={`p-4 rounded-xl border transition-all ${slotsFilled >= 1 ? 'bg-hover-bg border-border-default shadow-sm' : 'bg-surface-1 border-border-default text-text-tertiary'}`}>
              ...
              <div className={`p-4 rounded-xl border transition-all ${slotsFilled >= 2 ? 'bg-hover-bg border-border-default shadow-sm' : 'bg-surface-1 border-border-default text-text-tertiary'}`}>
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
              ...
              <div className={`p-4 rounded-xl border transition-all ${slotsFilled >= 3 ? 'bg-hover-bg border-border-default shadow-sm' : 'bg-surface-1 border-border-default text-text-tertiary'}`}>
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
            ...
            <div className="p-5 border-t border-border-default bg-surface-1">
              <button
                onClick={() => {
                  onClose();
                ...
                className="w-full py-3.5 bg-btn-main text-white rounded-xl text-[14px] font-bold hover:bg-btn-main-hover transition-colors disabled:opacity-50 disabled:bg-neutral-300 flex items-center justify-center gap-2 shadow-sm"
              >
                <Wand2 size={16} /> 生成方案
              </button>
            </div>
