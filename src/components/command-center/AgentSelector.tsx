...
export const AVAILABLE_AGENTS: Agent[] = [
 { id: 'core', name: 'Taptik 智能大脑', desc: '全局业务管家，可调度所有子节点智能体', icon: Bot, iconBg: 'bg-btn-main', iconColor: 'text-white' },
 { id: 'strategy', name: '策略专家', desc: '负责蓝海挖掘、竞品分析与选题规划', icon: Target, iconBg: 'bg-brand-light', iconColor: 'text-brand-logo' },
 { id: 'content', name: '全域内容打法团队', desc: '基于热点批量合成笔记，矩阵自动分发', icon: Sparkles, iconBg: 'bg-brand-light', iconColor: 'text-brand-logo' },
 { id: 'interaction', name: '客资转化智能体', desc: '监控全域消息，执行意向分级与转化 SOP', icon: MessageSquare, iconBg: 'bg-brand-light', iconColor: 'text-brand-logo' },
];
...
export const AgentSelector: React.FC<AgentSelectorProps> = ({ isOpen, onClose, activeAgentId, onSelectAgent, onOpenMarket }) => {
 ...
 className={`w-full text-left px-3 py-2.5 rounded-xl transition-all group flex items-center justify-between ${activeAgentId === agent.id ? 'bg-brand-light/50 text-brand-logo' : 'bg-transparent hover:bg-page-bg text-text-secondary'}`}
 >
 ...
 {activeAgentId === agent.id && <Check size={14} className="text-brand-logo" />}
 </button>
 ))}
 </div>
