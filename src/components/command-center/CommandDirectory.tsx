  ...
  return (
    ...
    >
      <div className="p-4 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-2 bg-white/5 px-3 py-2 rounded-xl flex-1 border border-white/5 focus-within:border-primary-500/50 transition-colors">
          ...
          <input 
            ...
            className="bg-transparent border-none outline-none text-[13px] font-bold text-white w-full placeholder:text-white/30"
          />
        </div>
      ...
      <div className="p-2 max-h-[420px] overflow-y-auto custom-scrollbar">
        {COMMAND_EXAMPLES.map(group => (
          <div key={group.category} className="mb-4">
            <div className="text-[11px] font-black tracking-widest text-white/40 px-3 py-2">
              {group.category}
            </div>
            <div className="px-1 space-y-1">
              {group.commands.map((item, idx) => (
                ...
                >
                  <div className="text-[13px] font-bold text-white group-hover:text-primary-400 transition-colors">
      ...
      <div className="p-4 bg-white/5 border-t border-white/10 text-center">
        <span className="text-[11px] font-bold text-white/40 italic">也可以在输入框自由用自然语言描述，我会尽力理解</span>
      </div>
    </motion.div>
  );
};
