               ...
               >
                  <div className="flex flex-col px-6 pt-6 pb-4 border-b border-neutral-100">
                     <div className="flex items-center justify-between mb-4">
                        <h2 className="text-[18px] font-black text-slate-900 tracking-tight">切换账号</h2>
                        <button 
                           onClick={onClose}
                     ...
                     <div className="relative">
                        ...
                        <input
                           ...
                           className="w-full h-10 pl-9 pr-4 bg-slate-50 border border-slate-200 focus:border-primary-500 outline-none rounded-xl text-[13px] text-slate-900 placeholder:text-slate-400 transition-colors font-bold"
                        />
                     </div>
                  </div>
                  ...
                  <div className="p-4 space-y-2 overflow-y-auto">
                     {filteredAccounts.length > 0 ? filteredAccounts.map((acc) => (
                        ...
                        >
                           ...
                           <div className="flex-1 min-w-0 flex flex-col">
                              <div className="flex items-center gap-1.5 mb-1">
                                 <span className="text-[14px] font-bold text-slate-900 truncate">{acc.label}</span>
                              </div>
                              <div className="text-[12px] text-slate-500 truncate flex items-center gap-2">
                                 {acc.industry && (
                                    <span className="shrink-0 bg-slate-100 text-slate-600 px-1.5 rounded-md text-[11px] font-black">{acc.industry}</span>
                                 )}
                        ...
                        <div className="py-8 text-center text-[13px] text-slate-400 font-bold">
                           没有找到匹配的账号
                        </div>
                     )}
                  </div>
                  ...
                  <div className="p-4 border-t border-slate-100 bg-slate-50 shrink-0">
                     <button className="w-full py-3 bg-white border border-slate-200 text-slate-700 rounded-xl text-[14px] font-bold hover:bg-slate-50 transition-colors">
                        添加其他账号
                     </button>
                  </div>
               </motion.div>
