...
export const SubagentChat: React.FC<SubagentChatProps> = ({ moduleId, moduleName, onNavigate, onClose, initialExpert, initialContext }) => {
 ...
 if (initialContext) {
 setContextPill({ type: initialExpert || '参考内容', text: initialContext });
 setInputValue((prev) => prev || `请分析这个`);
 }
 ...
 <span className="text-[10px] text-neutral-400 px-2 py-0.5 bg-neutral-100 rounded">编排计划 v1.0</span>
 ...
 {sub.status === 'completed' ? <Check size={12} strokeWidth={4} /> : <div className="text-[10px] ">{idx + 1}</div>}
 </div>
 <div>
 <div className="text-[12px] text-neutral-900 leading-tight">{sub.name}</div>
 <div className="text-[9px] text-neutral-400">{sub.agent} 执行中</div>
 </div>
 </div>
 ...
 <div>
 <h3 className="text-[14px] font-semibold text-neutral-900 leading-none">{currentExpert}</h3>
 ...
 <span className="text-[13px] text-neutral-900 group-hover:text-primary-500 transition-colors">{c.cmd}</span>
