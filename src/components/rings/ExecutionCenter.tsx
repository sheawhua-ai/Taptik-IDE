...
export const ExecutionCenter: React.FC = () => {
 ...
 <div>
 <h2 className="text-[17px] font-semibold text-neutral-900 tracking-tight">编排层：智策分发中心 (Orchestration)</h2>
 <p className="text-[11px] text-neutral-400">流水线助手: 负责 Agent 集群的任务排期、容错与闭环优化</p>
 </div>
 </div>
 ...
 className={`px-3 py-1.5 rounded-lg flex items-center gap-2 text-[11px] transition-all ${!isCompactView ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-400 hover:text-neutral-600'}`}
 >
 ...
 <h3 className="text-xl font-semibold text-neutral-900 tracking-tight flex items-center gap-2">
 ...
 {plan.errorState && (
 <div className="absolute top-0 right-0 px-6 py-2 bg-red-500 text-white text-[10px] uppercase tracking-widest rounded-bl-3xl z-10 flex items-center gap-2 shadow-lg">
 <AlertCircle size={14} /> {plan.errorState}
 </div>
 )}
 {plan.hitlState && (
 <div className="absolute top-0 right-0 px-6 py-2 bg-orange-500 text-white text-[10px] uppercase tracking-widest rounded-bl-3xl z-10 flex items-center gap-2 shadow-lg animate-pulse">
 ...
 {plan.errorState && (
 <button className="px-3 py-2 bg-red-500 text-white rounded-xl text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-lg hover:bg-red-600 transition-all">
 ...
 <span className="text-[12px] text-neutral-900">{plan.status}</span>
 </div>
 ...
 <p className="text-[11px] text-red-600">由于频繁调用，小红书 API 触发了 429 限流保护，建议暂停 15 分钟后重试。</p>
 ...
 <th className="px-6 py-4 text-[11px] text-neutral-400 uppercase tracking-widest">任务名称</th>
 <th className="px-6 py-4 text-[11px] text-neutral-400 uppercase tracking-widest">主责 Agent</th>
 <th className="px-6 py-4 text-[11px] text-neutral-400 uppercase tracking-widest">当前进度</th>
 <th className="px-6 py-4 text-[11px] text-neutral-400 uppercase tracking-widest">监控状态</th>
 <th className="px-6 py-4 text-[11px] text-neutral-400 uppercase tracking-widest">操作</th>
 </tr>
 </thead>
 ...
 <span className="text-[14px] text-neutral-900">{plan.title}</span>
 </div>
 </td>
 <td className="px-6 py-5">
 <span className="px-3 py-1 bg-neutral-100 rounded-lg text-[11px] text-neutral-500">{plan.agent}</span>
 </td>
 <td className="px-6 py-5">
 <div className="flex items-center gap-3">
 ...
 <td className="px-6 py-5">
 {plan.errorState ? (
 <span className="text-red-500 text-[11px] flex items-center gap-1"><AlertCircle size={12}/> {plan.errorState}</span>
 ) : plan.hitlState ? (
 <span className="text-orange-500 text-[11px] flex items-center gap-1 animate-pulse"><Pause size={12}/> HITL AWAITING</span>
 ) : (
 <span className="text-emerald-500 text-[11px] flex items-center gap-1"><RefreshCcw size={12} className="animate-spin" /> NORMAL</span>
 )}
 </td>
 <td className="px-6 py-5">
 ...
 <div>
 <h3 className="text-2xl font-semibold text-neutral-900 tracking-tight">{PLANS[activePlan].title}</h3>
 <p className="text-[13px] text-neutral-400 uppercase tracking-widest leading-relaxed">Agent 集群并行链路实时视图</p>
 </div>
 </div>
 ...
 >
 {node.status === 'hitl' && (
 <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-orange-500 text-white text-[9px] uppercase tracking-widest rounded-full shadow-lg z-20 animate-bounce">
 Action Required
 </div>
 )}
 ...
 <h4 className={`text-[17px] mb-2 ${node.status === 'pending' ? 'text-neutral-400' : 'text-neutral-900'}`}>{node.title}</h4>
 <p className="text-[12px] text-neutral-400 leading-relaxed">{node.desc}</p>
 
 {node.status === 'active' && (
 <div className="mt-6 pt-6 border-t border-neutral-100">
 <div className="flex items-center gap-2 text-[10px] text-primary-500 uppercase tracking-widest animate-pulse">
 ...
 <div className="flex items-center gap-2 text-[10px] text-orange-500 uppercase tracking-widest">
 ...
 <div>
 <p className="text-[12px] text-emerald-400 uppercase tracking-[0.2em]">质量决策引擎 (Closed Loop)</p>
 <h4 className="text-xl font-semibold">监测到笔记表现偏移</h4>
 </div>
 </div>
 
 <p className="text-[15px] text-neutral-300 leading-relaxed max-w-xl">
 监测到最近 <span className="text-white underline underline-offset-4 decoration-emerald-500">6 篇</span> 发布的笔记入池表现低于大盘预期。助手已通过研判模块，自动完成了以下闭环操作：
 </p>

 <div className="grid grid-cols-2 gap-4">
 <div className="p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm">
 <div className="text-[10px] text-neutral-500 uppercase tracking-widest mb-2">底层优化</div>
 <div className="text-[14px] ">#关键词权重调整</div>
 </div>
 <div className="p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm">
 <div className="text-[10px] text-neutral-500 uppercase tracking-widest mb-2">调度优化</div>
 <div className="text-[14px] ">延后非核心发布任务</div>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="text-[11px] text-neutral-300 font-mono pt-1 shrink-0">{log.time}</div>
 <div className={`text-[13px] ${log.type === 'success' ? 'text-emerald-700' : log.type === 'warning' ? 'text-orange-600' : 'text-neutral-500'}`}>
 {log.msg}
 </div>
 </div>
 ))}
 ...
 <div>
 <h3 className="text-2xl font-semibold text-neutral-900 tracking-tight">人机校验审批中心</h3>
 <p className="text-[13px] text-neutral-400 uppercase tracking-widest">待办事项：夏季大促爆文文案初稿确认 (Node 03)</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-[10px] text-neutral-400 uppercase tracking-widest">当前运行状态</p>
 <p className="text-[15px] text-orange-500 flex items-center gap-2">
 <Pause size={16} fill="currentColor" /> 任务流已自动挂起，等待人工核准
 </p>
 </div>
 </div>
 <div className="text-right">
 <p className="text-[10px] text-neutral-400 uppercase tracking-widest">预计延时风险</p>
 <p className="text-[15px] text-neutral-900">12m 45s</p>
 </div>
 </div>
 ...
 <div className="flex items-center gap-3">
 <span className="px-3 py-1 bg-primary-500 text-white text-[10px] rounded-full uppercase">小红书风格</span>
 <span className="px-3 py-1 bg-neutral-100 text-neutral-500 text-[10px] rounded-full uppercase">夏季大促主题</span>
 </div>
 <h5 className="text-xl font-semibold leading-tight text-neutral-900">标题：救命！这款宠物粮真的让挑嘴猫从此开启“炫饭”模式！😭🐾</h5>
 <p className="text-[15px] text-neutral-600 leading-relaxed">
 家人们谁懂啊！最近入手的这款「宠味巡航」定制粮真的绝了！之前试了好几个大牌，主子都爱答不理，这次竟然直接光盘！... (此处省略 800 字正文)
 </p>
 <div className="grid grid-cols-3 gap-4 h-48">
 <div className="bg-neutral-100 rounded-3xl border border-neutral-200 flex items-center justify-center text-neutral-400 text-[10px] uppercase">封面图 A - AI 已生成</div>
 <div className="bg-neutral-100 rounded-3xl border border-neutral-200 flex items-center justify-center text-neutral-400 text-[10px] uppercase">展示图 B - AI 已生成</div>
 <div className="bg-neutral-100 rounded-3xl border border-neutral-200 flex items-center justify-center text-neutral-400 text-[10px] uppercase">细节图 C - AI 已生成</div>
 </div>
 </div>
 </div>
 ...
 <p className="text-[11px] text-neutral-400 uppercase tracking-widest">操作审计已开启，所有动作由管理员 01 执行</p>
 </div>
 </div>
 </motion.div>
 </>
