  ...
  const PLANS = [
    { 
        ...
        nodes: [
            ...
            { step: '02', title: '任务编排', desc: 'AI 正基于选题逻辑重组', icon: Workflow, status: 'active' },
            { step: '03', title: '人机校验', desc: '待生成的文案质量风控', icon: ShieldCheck, status: 'pending' },
    ...
    { 
        ...
        nodes: [
            ...
            { step: '02', title: '词云过滤', desc: '剔除高竞争大词', icon: Workflow, status: 'active' },
      ...
      <div className="flex-1 flex overflow-hidden bg-neutral-50/20">
          ...
          <div className="max-w-6xl mx-auto space-y-12">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {PLANS.map((plan, i) => (
                        <div key={i} onClick={() => setActivePlan(i)} className={`p-8 bg-white rounded-[40px] border transition-all cursor-pointer group ${activePlan === i ? 'border-primary-500 shadow-2xl scale-[1.02]' : 'border-neutral-100 hover:border-neutral-200 shadow-sm'}`}>
                            <div className="flex items-center justify-between mb-8">
                                <div className="space-y-1">
                                    <h4 className="text-lg font-black text-neutral-900">{plan.title}</h4>
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${plan.id === 0 ? 'bg-primary-500' : 'bg-orange-500'} animate-pulse`} />
                                        <span className="text-[11px] font-black text-neutral-400 uppercase tracking-widest">{plan.agent} | {plan.status} 处理中</span>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <button className="w-10 h-10 rounded-xl bg-neutral-50 flex items-center justify-center text-neutral-400 hover:text-neutral-900 transition-colors">
                                        <Pause size={18} />
                                    </button>
                                    <button className="w-10 h-10 rounded-xl bg-neutral-50 flex items-center justify-center text-neutral-400 hover:text-neutral-900 transition-colors">
                                        <X size={18} />
                            ...
                            <div className="flex items-center gap-4 mb-8">
                                <div className="flex-1 h-2 bg-neutral-100 rounded-full overflow-hidden p-0.5">
                                    <div className={`h-full ${plan.id === 0 ? 'bg-primary-500 shadow-[0_0_10px_rgba(230,53,96,0.5)]' : 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.5)]'} rounded-full`} style={{ width: plan.status }} />
                                </div>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {plan.nodes.map((node, j) => (
                                    <div key={j} className={`h-1.5 rounded-full ${node.status === 'completed' ? 'bg-emerald-500' : node.status === 'active' ? (plan.id === 0 ? 'bg-primary-500' : 'bg-orange-500') : 'bg-neutral-100'}`} />
                                ))}
                            </div>
                        </div>
                    ))}
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-neutral-100 rounded-[48px] p-12 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 relative">
                        ...
                        {PLANS[activePlan].nodes.map((node, i) => (
                            <div key={i} className={`relative z-10 bg-white border p-8 rounded-[40px] transition-all duration-500 shadow-sm ${node.status === 'active' ? 'border-primary-500 ring-4 ring-primary-500/5' : 'border-neutral-100'}`}>
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${node.status === 'completed' ? 'bg-emerald-500 text-white' : node.status === 'active' ? 'bg-neutral-900 text-white animate-pulse' : 'bg-neutral-50 text-neutral-300'}`}>
        ...
        <div className="w-[360px] border-l border-neutral-100 bg-white h-full z-10 shrink-0">
           <DigitalEmployeeProgress 
             moduleName="编排中心"
             tasks={[
               { id: '1', name: '集群算力自检', status: 'completed', agent: '系统守卫', time: '08:00' },
               { id: '2', name: '全链路容错重试', status: 'running', agent: '调度 Agent', time: '09:45' },
               { id: '3', name: '节点性能压测', status: 'pending', agent: '质量 Agent', time: '11:30' },
             ]}
           />
        </div>
      </div>
