...
export const ContentProduction: React.FC<{ hasData?: boolean }> = ({ hasData = true }) => {
 ...
 <p className="text-[11px] text-neutral-400 mt-0.5">管理自有资产矩阵，并将生成的素材下发至各渠道账号发布</p>
 </div>
 </div>
 <div className="flex items-center gap-2 bg-neutral-100 p-1.5 rounded-2xl">
 <button 
 onClick={() => setViewMode('accounts')}
 className={`px-5 py-2.5 rounded-xl text-[12px] transition-all ${viewMode === 'accounts' ? 'bg-white shadow-xl text-neutral-900 border border-neutral-200/50' : 'text-neutral-400'}`}
 >账号池管理</button>
 <button 
 onClick={() => setViewMode('distribution')}
 className={`px-5 py-2.5 rounded-xl text-[12px] transition-all ${viewMode === 'distribution' ? 'bg-white shadow-xl text-neutral-900 border border-neutral-200/50' : 'text-neutral-400'}`}
 >待分发素材队列</button>
 </div>
 </div>
 ...
 <h3 className="text-xl font-semibold text-neutral-900">自有资产与 KOS 员工号库</h3>
 <span className="text-[11px] text-neutral-400 bg-primary-50 text-primary-500 border border-primary-100 px-3 py-1 rounded-full uppercase tracking-widest">完全掌控授权态</span>
 </div>
 ...
 <div className="flex items-center gap-4">
 <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${account.health === 'good' ? 'bg-primary-50 text-primary-500' : account.health === 'warning' ? 'bg-amber-50 text-amber-500' : 'bg-rose-50 text-rose-500'}`}>
 {account.health === 'error' ? <AlertCircle size={20} /> : <User size={20} />}
 </div>
 <div>
 <div className="flex items-center gap-2 mb-0.5">
 <h4 className="text-[15px] font-semibold text-neutral-900">{account.name}</h4>
 <span className="px-1.5 py-0.5 rounded border border-neutral-200 text-[9px] text-neutral-400">{account.platform}</span>
 </div>
 <p className="text-[11px] text-neutral-400 tracking-wide flex items-center gap-2">
 {account.type} <span className="w-1 h-1 rounded-full bg-neutral-200"></span> 粉丝 {account.followers}
 </p>
 </div>
 </div>
 <div className="text-right">
 <span className={`px-2 py-1 rounded-md text-[10px] uppercase tracking-widest ${account.health === 'good' ? 'bg-success-50 text-success-600' : account.health === 'warning' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-500'}`}>
 {account.status}
 </span>
 </div>
 </div>
 ...
 <p className="text-[10px] text-neutral-400 uppercase tracking-widest mb-0.5">待执行发布</p>
 <p className={`text-[14px] ${account.taskTodo > 0 ? 'text-primary-500' : 'text-neutral-900'}`}>{account.taskTodo} 篇</p>
 </div>
 <div className="w-px h-8 bg-neutral-200"></div>
 <div className="flex-1">
 <p className="text-[10px] text-neutral-400 uppercase tracking-widest mb-0.5">配额</p>
 <p className="text-[14px] text-neutral-900">{account.quota}</p>
 </div>
 <div className="w-px h-8 bg-neutral-200"></div>
 <div className="flex-1">
 <p className="text-[10px] text-neutral-400 uppercase tracking-widest mb-0.5">累计分发</p>
 <p className="text-[14px] text-neutral-900">{account.taskDone} 篇</p>
 </div>
 ...
 <div>
 <div className="flex items-center justify-between mb-6">
 <h3 className="text-xl font-semibold text-neutral-900">外部合作与素人 KOC 监控池</h3>
 <span className="text-[11px] text-neutral-400 bg-neutral-100 px-3 py-1 rounded-full uppercase tracking-widest">爬虫公开态监控</span>
 </div>
 ...
 <div className="w-12 h-12 bg-neutral-50 rounded-xl flex items-center justify-center text-neutral-400 shrink-0 relative">
 <ScanLine size={20} />
 ...
 <h4 className="text-[15px] font-semibold text-neutral-900">{koc.name}</h4>
 <a href="#" className="text-neutral-300 hover:text-blue-500 transition-colors"><Link size={12} /></a>
 </div>
 <p className="text-[10px] text-neutral-500 tracking-widest flex items-center gap-1.5 break-all">
 ...
 <h3 className="text-xl font-semibold text-neutral-900">分发队列暂无数据</h3>
 <p className="text-[13px] text-neutral-400 mt-2">请先在「项目与内容」中完成笔记生成，再由系统推送到此列队</p>
 </div>
 )}
 </div>
 </div>
