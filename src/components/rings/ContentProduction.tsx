...
export const ContentProduction: React.FC<{ hasData?: boolean }> = ({ hasData = true }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden w-full relative">
      {/* Header */}
      <div className="h-20 border-b border-border-default px-8 flex items-center justify-between shrink-0 bg-surface-1 z-10 sticky top-0 w-full">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-btn-main text-white rounded-xl flex items-center justify-center shadow-lg">
            <Share2 size={24} />
          </div>
          <div>
      ...
      <div className="flex-1 w-full overflow-y-auto bg-page-bg custom-scrollbar pb-24">
          ...
          {viewMode === 'dashboard' ? (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div 
                  className="bg-surface-1 p-5 rounded-[20px] border border-border-default shadow-sm flex flex-col justify-between cursor-pointer hover:border-primary-200 transition-colors"
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {MOCK_ACCOUNTS.map((acc) => (
                    <div key={acc.id} className="bg-surface-1 border border-border-default rounded-[24px] p-5 shadow-sm hover:shadow-md hover:border-primary-200 transition-all">
                      {/* Header */}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-0.5">
                              <h4 className="text-[15px] font-semibold text-text-main">{acc.name}</h4>
            ...
            <div className="space-y-6">
              {MOCK_QUEUE.map(task => (
                <div key={task.id} className="bg-surface-1 border border-border-default rounded-[20px] p-5 shadow-sm flex items-center justify-between hover:shadow-md hover:border-primary-200 transition-all">
                  ...
                  <div className="flex items-center gap-3 shrink-0 ml-8">
                    {task.status === '等待发文' ? (
                      ...
                      <span className="px-4 py-2 text-text-main font-medium text-[13px] flex items-center gap-1.5">
      ...
      {showQrCode && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowQrCode(null)}>
          <div className="w-[320px] bg-surface-1 rounded-[24px] shadow-2xl p-6 flex flex-col items-center" onClick={e => e.stopPropagation()}>
            <div className="w-12 h-12 bg-brand-light text-brand-logo rounded-full flex items-center justify-center mb-4">
              <QrCode size={24} />
            </div>
            ...
            <button onClick={() => setShowQrCode(null)} className="w-full py-3 bg-btn-main text-white rounded-[14px] text-[13px] font-medium hover:bg-btn-main-hover transition-colors">
              完成扫码
            </button>
          </div>
        </div>
