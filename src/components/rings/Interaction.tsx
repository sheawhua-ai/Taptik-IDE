    ...
    return (
      <div className="flex h-full w-full bg-white overflow-hidden items-center justify-center">
         <div className="max-w-md w-full p-12 bg-white rounded-[64px] border border-neutral-100 shadow-2xl shadow-neutral-200/50 flex flex-col items-center text-center">
            ...
            <h3 className="text-2xl font-black text-neutral-900 mb-4 italic tracking-tight">客资与工单已上线</h3>
      ...
      <div className="w-[450px] border-r border-neutral-100 flex flex-col h-full bg-[#fafafa]">
        <div className="p-8 border-b border-neutral-100 bg-white">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-xl font-black text-neutral-900 tracking-tight">客资与工单中心</h2>
               ...
               >
                 {item.urgency === 'high' && (
                    <div className="absolute top-5 right-5 flex items-center gap-1 bg-red-50 px-2 py-1 rounded-lg border border-red-100">
                       ...
                       <span className="text-[10px] font-black text-red-500">紧急跟进</span>
                    </div>
                 )}
                 <div className="flex items-center gap-3 mb-3 pr-16">
                   ...
                   <div>
                     <div className="text-[14px] font-black text-neutral-900 leading-tight mb-1">{item.title}</div>
                         <div className="flex items-center gap-2">
                           <span className="text-[10px] font-bold text-neutral-400 capitalize">{item.source}</span>
                         </div>
                       </div>
                     </div>
                     <p className="text-[13px] font-bold text-neutral-600 mb-3 line-clamp-2 leading-relaxed">
                       {item.description}
                     </p>
                     <div className="flex items-center justify-between pt-3 border-t border-neutral-50">
                         <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">{item.time}</span>
                         <span className={`text-[11px] font-black ${item.status === 'pending' ? 'text-blue-500' : 'text-emerald-500'}`}>
          ...
          <div className="flex-1 flex flex-col bg-white">
            <div className="h-20 border-b border-neutral-100 px-8 flex items-center justify-between shrink-0 bg-white">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-[16px] font-black text-neutral-900 tracking-tight mb-1">{selectedTask.title}</h3>
                  <div className="flex items-center gap-2">
                      <span className="text-[11px] font-bold text-neutral-400">来自: {selectedTask.source}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                 <button className="px-5 py-2 bg-neutral-900 text-white rounded-xl text-[12px] font-black flex items-center gap-2 hover:bg-neutral-800 shadow-xl shadow-neutral-200">
            ...
            <div className="flex-1 overflow-y-auto p-12 bg-neutral-50/30 custom-scrollbar flex flex-col items-center">
              ...
              <div className="w-full max-w-4xl border border-neutral-200 bg-white rounded-[32px] p-8 shadow-md relative overflow-hidden">
                 ...
                 <div className="flex items-center gap-4 mb-8 pb-6 border-b border-neutral-100">
                    ...
                    <div>
                        <h4 className="text-[15px] font-black text-neutral-900 mb-1">AI 诊断分析</h4>
                        <p className="text-[12px] font-bold text-neutral-500">{selectedTask.description}</p>
                    </div>
                 </div>
                    ...
                    <div className="space-y-6">
                       <div className="p-5 bg-blue-50 border border-blue-100 rounded-2xl flex gap-4 relative">
                          ...
                          <div className="flex-1">
                             <h5 className="text-[13px] font-black text-blue-900 mb-2">识别到的高权重互动评论：</h5>
                             {selectedTask.snapshots?.map((snap, idx) => (
                                <p key={idx} className="text-[13px] font-bold text-blue-900/80 mb-3 bg-white p-3 rounded-lg border border-blue-100 shadow-sm">{snap}</p>
                             ))}
                          </div>
                       </div>
                       
                       <div>
                          <h5 className="text-[13px] font-black text-neutral-900 mb-3 flex items-center gap-2"><Bot size={16} className="text-primary-500"/> AI推荐回复话术 (已通过合规过滤)</h5>
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                              <button className="px-6 py-2 bg-white text-neutral-600 border border-neutral-200 rounded-xl text-[12px] font-black hover:bg-neutral-50">
                                 复制并前往平台回复
                              </button>
                              <button className="px-6 py-2 bg-neutral-900 text-white rounded-xl text-[12px] font-black flex items-center gap-2 hover:bg-primary-500 shadow-xl shadow-neutral-200/50">
                 ...
                 {selectedTask.type === 'dm' && (
                    <div className="space-y-6">
                       <div className="p-6 bg-[#fafafa] border border-neutral-100 rounded-3xl relative">
                           ...
                           <h5 className="text-[13px] font-black text-neutral-900 mb-4 flex items-center gap-2"><Smartphone size={16} className="text-purple-500"/> 最新私信存档内容</h5>
                       ...
                       <div className="grid grid-cols-2 gap-4 mt-4">
                           <div className="p-5 border border-purple-100 bg-purple-50/50 rounded-2xl">
                              <h6 className="text-[11px] font-black text-purple-900 uppercase tracking-widest mb-1">意向等级与诉求拆解</h6>
                              <div className="flex items-center gap-2 mt-2">
                                 <div className="flex items-center justify-center bg-purple-100 text-purple-700 px-2 py-1 rounded-md text-[10px] font-black">加盟咨询</div>
                                 <div className="flex items-center justify-center bg-emerald-100 text-emerald-700 px-2 py-1 rounded-md text-[10px] font-black">意向极高</div>
                              </div>
                           </div>
                           <div className="p-5 border border-purple-100 bg-purple-50/50 rounded-2xl">
                              <h6 className="text-[11px] font-black text-purple-900 uppercase tracking-widest mb-1">已提取的关键客资</h6>
                              <div className="mt-2 text-[13px] font-bold text-neutral-900">
                       ...
                       <div className="flex justify-end pt-4">
                          <button 
                             ...
                             className={`px-8 py-3 rounded-xl text-[13px] font-black flex items-center gap-2 transition-all shadow-xl ${isCRMTransferred ? 'bg-emerald-500 text-white shadow-emerald-500/20' : 'bg-[#008CEE] text-white hover:bg-[#0077CC] shadow-[#008CEE]/20'}`}
                          >
                             {isCRMTransferred ? (
                                <>
