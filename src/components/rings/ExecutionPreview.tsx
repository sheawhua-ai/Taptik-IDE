...
export function ExecutionPreview({ onStart, onBack, defaultDirection = "幼犬换粮避坑", defaultDesc = "这是当前最适合该商家的自然流切入点。低粉爆款信号正在增强，且品牌卖点可以自然植入。建议直接做内容矩阵铺设，先不进行硬广投流。" }: ExecutionPreviewProps) {
  ...
  useEffect(() => {
    const handleAdopt = (e: any) => {
      const { adjustType, name } = e.detail;
      if (adjustType === "direction") setConfirmDirection(name);
      if (adjustType === "group") setConfirmGroup(name);
      if (adjustType === "dist") setConfirmDist(name);
      if (adjustType === "sync") setConfirmSync(name);
    };
    window.addEventListener("adopt-alternative", handleAdopt);
    return () => window.removeEventListener("adopt-alternative", handleAdopt);
  }, []);
  ...
  const getSubAgentContent = (type: string) => {
    switch (type) {
      case "direction":
        return {
          title: "调整主方向",
          subtitle: "已基于当前商家画像、内容目标和账号池生成 3 个新方案",
          alternatives: [
            { id: "A", name: "软便过渡期清单", desc: "更适合自然流" },
            { id: "B", name: "换粮失败案例复盘", desc: "话题更具冲突感" },
            {
              id: "C",
              name: "幼犬冻干平替测评",
              desc: "更适合测评号，不适合官方号",
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 relative items-start w-full">
      <motion.div
        ...
        className="bg-[#fcfcfc] rounded-2xl border border-border-default shadow-xl overflow-hidden transition-all duration-300 w-full"
      >
        {/* 1. 执行摘要卡 */}
        <div className="bg-surface-1 p-8 border-b border-border-default relative">
          ...
          <div className="bg-page-bg border border-border-default rounded-xl p-5">
            <p className="text-[15px] text-text-main leading-relaxed font-medium">
              我将启动一个{" "}
              <strong className="text-text-main">7 天搜索卡位</strong> 项目：
              <br />
              围绕「<strong className="text-text-main">{confirmDirection}</strong>
              」生成 <strong className="text-text-main">12</strong> 篇内容，
              <br />
              其中 8 篇优先分配给自有账号，4 篇进入外部素人领取池。
            </p>
            <div className="mt-3 pt-3 border-t border-border-default/60 text-[13px] text-text-tertiary">
              {defaultDesc}
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-10">
          ...
          <div>
            <h4 className="text-[16px] font-bold text-text-main mb-4 flex items-center gap-2">
              <Layers className="text-brand-logo" size={18} /> 智能 方案拆解
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <div className="bg-surface-1 border border-border-default rounded-xl p-5 shadow-sm md:col-span-2 relative group">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-page-bg border border-border-default rounded-xl p-3">
            ...
            <div className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="bg-surface-1 border border-border-default rounded-xl p-5 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 border border-border-default rounded-xl hover:border-border-default transition-colors group">
                    <div>
              ...
              <div className="bg-surface-1 border border-border-default rounded-xl p-5 shadow-sm flex flex-col">
                ...
                <div className="flex-1 flex flex-col items-center justify-center bg-page-bg rounded-xl border border-dashed border-border-default p-6 text-center">
                  ...
                  <div className="text-[13px] text-text-tertiary leading-relaxed max-w-sm">
                    将进入外部素人任务大厅。
                    <br />
          ...
          <div>
            <h4 className="text-[16px] font-bold text-text-main mb-4 flex items-center gap-2">
              <CheckCircle2 className="text-text-main" size={18} />{" "}
              请确认以下关键判断
            </h4>
            <div className="space-y-3">
              {/* 判断 1 */}
              <div className="bg-surface-1 border border-border-default rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
          ...
          {/* 4. 执行边界与启动 */}
          <div className="border-t border-border-default pt-8 pb-4">
            <div className="flex flex-wrap items-center gap-4">
              <button
                onClick={onStart}
                className="px-8 py-3.5 bg-btn-main hover:bg-btn-main-hover text-white rounded-xl text-[14px] font-bold shadow-lg shadow-neutral-900/20 flex items-center gap-2 transition-all active:scale-95"
              >
                确认方案并开始生成 <ArrowRight size={16} />
              </button>
              <button
                onClick={onBack}
                className="px-6 py-3.5 bg-surface-1 border border-border-default hover:bg-page-bg text-text-secondary rounded-xl text-[14px] font-medium transition-colors"
              >
                返回调整建议
              </button>
            </div>
