    ...
    <div className="flex flex-col h-full bg-white overflow-hidden">
      <div className="h-20 border-b border-neutral-100 px-8 flex items-center justify-between shrink-0 bg-white z-10">
        <div className="flex items-center gap-4">
           ...
           <div>
              <h2 className="text-[17px] font-black text-neutral-900 tracking-tight">私域承接与 CRM</h2>
              <p className="text-[11px] font-bold text-neutral-400">线索全生命周期管理，打通笔记到成交的最后一公里</p>
           </div>
        </div>
                  ...
                  <div className="flex gap-1 bg-white p-1 rounded-2xl border border-neutral-100">
                     {[
                       ...
                       <button 
                          ...
                          className={`px-6 py-2.5 rounded-xl text-[13px] font-black transition-all flex items-center gap-3 ${pipelineTab === tab.id ? 'bg-neutral-900 text-white shadow-lg' : 'text-neutral-400 hover:text-neutral-600'}`}
                       >
                          {tab.name}
                  ...
                  <div className="flex items-center gap-4">
                     <div className="relative">
                        ...
                        <input 
                           ...
                           className="pl-10 pr-4 py-2 bg-white border border-neutral-200 rounded-xl text-[12px] font-bold outline-none focus:border-primary-500 transition-all w-64"
                        />
                     </div>
                  ...
                  {LEADS.filter(l => l.status === pipelineTab || pipelineTab === 'new').map(lead => (
                    <div key={lead.id} className="group bg-white p-6 rounded-[32px] border border-neutral-100 hover:border-primary-500 hover:shadow-2xl hover:shadow-neutral-200/50 transition-all flex items-center gap-8">
                       ...
                       <div className="flex-1 grid grid-cols-4 gap-8">
                          <div>
                             <h4 className="text-[15px] font-black text-neutral-900 mb-1">{lead.name}</h4>
                             <p className="text-[12px] font-bold text-neutral-400 flex items-center gap-1"><Phone size={12}/> {lead.phone}</p>
                          </div>
                          
                          <div>
                             <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">意向维度</p>
                             <div className="flex flex-wrap gap-1">
                                <span className="px-2 py-0.5 bg-blue-50 text-blue-500 text-[10px] font-black rounded border border-blue-100">{lead.product}</span>
                                <span className="px-2 py-0.5 bg-neutral-50 text-neutral-400 text-[10px] font-black rounded border border-neutral-100">{lead.budget}</span>
                             </div>
                          </div>

                          <div>
                             <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">来源笔记</p>
                             <p className="text-[13px] font-bold text-neutral-700 truncate max-w-[150px]">{lead.source}</p>
                          </div>

                          <div className="flex items-center gap-6">
                             <div className="text-right flex-1">
                                <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">停留时长</p>
                                <p className="text-[14px] font-black text-neutral-900">{lead.time}</p>
                             </div>
                             {lead.status === 'new' && (
                       ...
                       <div className="flex gap-2">
                          <button className="px-6 py-2.5 bg-neutral-900 text-white rounded-2xl text-[12px] font-black hover:bg-primary-500 transition-all opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0">
                             立即联系
                          </button>
