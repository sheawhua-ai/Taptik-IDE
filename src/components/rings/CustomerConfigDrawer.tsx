    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className="fixed inset-0 bg-hover-bg z-[200] flex flex-col"
        >
          {/* Header */}
          <div className="bg-surface-1 border-b border-border-default h-14 shrink-0 px-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h1 className="text-[16px] font-bold text-text-main">客户参与配置</h1>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-[400px] border-r border-border-default bg-page-bg flex flex-col">
              <div className="p-4 border-b border-border-default bg-surface-1">
                <div className="text-[13px] font-bold text-text-secondary mb-2">回答分支切换</div>
              ...
              <div className="flex-1 overflow-y-auto p-8 flex justify-center custom-scrollbar">
                {/* Phone mockup */}
                <div className="w-[320px] h-[650px] bg-surface-1 rounded-[40px] shadow-2xl border-[8px] border-neutral-800 overflow-hidden relative flex flex-col">
                  ...
                  <div className="flex-1 bg-page-bg p-4 -mt-4 rounded-t-2xl overflow-y-auto custom-scrollbar">
                    <div className="space-y-4">
                      <div className="bg-surface-1 p-4 rounded-xl shadow-sm border border-border-default">
                        <div className="text-[13px] font-bold text-text-main mb-2">1. 狗狗当前的年龄是？</div>
                        <div className="space-y-2">
                          <button className="w-full text-left px-4 py-2 border border-primary-500 bg-brand-light text-primary-700 rounded-lg text-[13px] font-medium flex justify-between items-center">
                            3-6个月
                            <CheckCircle size={16} />
                          </button>
                          <button className="w-full text-left px-4 py-2 border border-border-default text-text-secondary rounded-lg text-[13px]">6个月以上</button>
                        </div>
                      </div>
                      
                      <div className="bg-surface-1 p-4 rounded-xl shadow-sm border border-border-default">
                        <div className="text-[13px] font-bold text-text-main mb-2">2. 换粮过程中遇到了什么问题？(单选)</div>
                        <div className="space-y-2">
                          <button className="w-full text-left px-4 py-2 border border-border-default text-text-secondary rounded-lg text-[13px]">便便变软或拉稀</button>
                          <button className="w-full text-left px-4 py-2 border border-primary-500 bg-brand-light text-primary-700 rounded-lg text-[13px] font-medium flex justify-between items-center">
                            挑食不爱吃
                            <CheckCircle size={16} />
                          </button>
                          <button className="w-full text-left px-4 py-2 border border-border-default text-text-secondary rounded-lg text-[13px]">一切正常</button>
                        </div>
                      </div>
                      
                      <button className="w-full py-3 bg-btn-main text-white rounded-xl text-[14px] font-bold">
                        生成我的专属笔记
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex-1 flex flex-col bg-surface-1">
                ...
                <div className="max-w-2xl mx-auto space-y-8">
                  ...
                  <div>
                    <h3 className="text-[16px] font-bold text-text-main mb-4 flex items-center gap-2">
                      <Bot size={18} className="text-brand-logo" /> 当前配置方案
                    </h3>
                    <div className="space-y-4">
              ...
              {/* AI Chat Box */}
              <div className="shrink-0 border-t border-border-default bg-surface-1 p-6 pb-24">
                <div className="max-w-2xl mx-auto">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-page-bg border border-border-default rounded-xl pl-4 pr-12 py-3.5 text-[14px] focus:outline-none focus:border-primary-400 focus:bg-surface-1 transition-colors"
                    />
                    <button 
                      onClick={() => {
          ...
          {/* Fixed Bottom Bar */}
          <div className="absolute bottom-0 inset-x-0 h-16 bg-surface-1 border-t border-border-default flex items-center justify-between px-8 z-10 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
            <div className="text-[13px] text-text-tertiary flex items-center gap-2">
              <Info size={16} /> 确认配置后将生成{planAmount}个待领取名额和1个消费者H5入口
            </div>
            <div className="flex items-center gap-3">
              <button className="px-6 py-2.5 bg-surface-1 border border-border-default text-text-secondary rounded-lg text-[14px] font-bold hover:bg-page-bg transition-colors">
                保存项目草稿
              </button>
              <button 
                onClick={onConfirm}
                className="px-6 py-2.5 bg-btn-main text-white rounded-lg text-[14px] font-bold hover:bg-btn-main-hover transition-colors shadow-sm"
              >
                确认配置并创建项目
              </button>
            </div>
