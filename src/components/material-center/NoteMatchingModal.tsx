      ...
      <div className="bg-surface-1 rounded-2xl max-w-4xl w-full overflow-hidden shadow-2xl border border-border-default animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[88vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded bg-surface-1/15 text-white text-[13px] font-extrabold">
                  不显示百分比置信度
                </span>
              </div>
        ...
        <div className="p-4 bg-page-bg border-b border-border-default/80 flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 rounded bg-btn-main text-white text-[13px] font-extrabold">
                当前笔记草稿
              </span>
          ...
          <button
            onClick={handleScan}
            disabled={isScanning}
            className="px-4 py-2 bg-btn-main hover:bg-btn-main-hover text-white rounded-xl text-[13px] font-extrabold flex items-center gap-1.5 shadow-2xs active:scale-95 transition-all shrink-0"
          >
        ...
        <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-12">
          ...
          <div className="md:col-span-4 border-r border-border-default bg-page-bg p-4 space-y-2 overflow-y-auto">
            <div className="text-[13px] font-black text-text-tertiary uppercase px-1">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {isConfirmed ? (
                      <span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 text-[13px] font-bold flex items-center gap-1">
                        <Check size={11} /> 已选
                      </span>
                    ) : pos.matchedLevel === 'recommend' ? (
                      <span className="px-2 py-0.5 rounded bg-brand-light text-primary-700 border border-primary-200 text-[13px] font-bold">
                        首选推荐
                      </span>
                    ) : pos.matchedLevel === 'other' ? (
                      <span className="px-2 py-0.5 rounded bg-hover-bg text-text-secondary border border-border-default text-[13px] font-bold">
                        其他可选
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded bg-rose-50 text-danger border border-danger-light text-[13px] font-bold">
                        无合适素材
                      </span>
                    )}
                  </div>
                  <p className="text-[13px] font-medium text-text-secondary line-clamp-2">
                    {pos.requirementDesc}
                  </p>
                </div>
              );
          ...
          <div className="md:col-span-8 p-6 overflow-y-auto space-y-6">
            {isScanning ? (
              <div className="h-full flex flex-col items-center justify-center p-12 text-center space-y-3">
                ...
                <div className="text-[13px] text-text-tertiary max-w-sm">
              ...
              <>
                ...
                {currentPos.matchedLevel === 'recommend' && currentPos.matchedAssetId && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-lg bg-primary-600 text-white font-extrabold text-[13px]">
                        推荐
                      </span>
                        ...
                        <div className="p-4 rounded-xl border border-primary-200 bg-brand-light/30 space-y-3">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-2 flex-1 min-w-0">
                              ...
                              <div className="p-2.5 bg-surface-1 rounded-xl border border-border-default/80 text-[13px] text-text-secondary space-y-1">
                          ...
                          <div className="flex items-center justify-end gap-2 pt-1">
                            <button
                              onClick={() => onViewAssetDetail(asset)}
                              className="px-3.5 py-1.5 rounded-xl bg-surface-1 hover:bg-hover-bg border text-text-secondary font-bold text-[13px] flex items-center gap-1"
                            >
                              <Eye size={14} /> 查看大图与历史
                            </button>
                            <button
                              onClick={() => handleConfirmAsset(currentPos.posIndex, asset)}
                              className={`px-4 py-1.5 rounded-xl font-extrabold text-[13px] flex items-center gap-1.5 shadow-2xs transition-all ${
                                isConfirmed
                ...
                {currentPos.matchedLevel === 'other' && currentPos.matchedAssetId && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-lg bg-neutral-200 text-text-secondary font-extrabold text-[13px]">
                        其他可选
                      </span>
                      <span className="text-[13px] font-bold text-text-secondary">
                        能够满足部分需求，但存在明显取舍的素材
                        ...
                        <div className="p-4 rounded-xl border border-border-default bg-surface-1 space-y-3">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-2 flex-1 min-w-0">
                              ...
                              <div className="p-2.5 bg-page-bg rounded-xl border border-border-default/80 text-[13px] text-text-secondary space-y-1">
                          ...
                          <div className="flex items-center justify-end gap-2 pt-1">
                            <button
                              onClick={() => onViewAssetDetail(asset)}
                              className="px-3.5 py-1.5 rounded-xl bg-hover-bg hover:bg-selected-bg text-text-secondary font-bold text-[13px] flex items-center gap-1"
                            >
                              <Eye size={14} /> 查看大图
                            </button>
                            <button
                              onClick={() => handleConfirmAsset(currentPos.posIndex, asset)}
                              className={`px-4 py-1.5 rounded-xl font-extrabold text-[13px] flex items-center gap-1.5 transition-all ${
                                isConfirmed
                                  ? 'bg-emerald-600 text-white'
                                  : 'bg-btn-main hover:bg-btn-main-hover text-white active:scale-95'
                              }`}
                ...
                {currentPos.matchedLevel === 'none' && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-lg bg-rose-100 text-danger font-extrabold text-[13px]">
                        暂无合适素材
                      </span>
                      <span className="text-[13px] font-bold text-text-tertiary">
                        不强行推荐低质量结果
                    ...
                    <div className="p-6 rounded-xl border border-danger-light bg-rose-50/50 space-y-4 text-center">
                      ...
                      <div className="flex items-center justify-center gap-2.5 pt-2">
                        <button
                          ...
                          className="px-4 py-2 bg-btn-main hover:bg-btn-main-hover text-white rounded-xl text-[13px] font-extrabold flex items-center gap-1.5 shadow-2xs active:scale-95"
                        >
                          <Plus size={15} /> 创建补拍任务
                        </button>
                        <button
                          onClick={() => alert('已临时放宽该插图位置的严格限定要求')}
                          className="px-4 py-2 bg-surface-1 border border-neutral-300 hover:bg-page-bg text-text-main rounded-xl text-[13px] font-bold"
                        >
                          调整图片要求
                        </button>
                      </div>
