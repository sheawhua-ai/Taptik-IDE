    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-surface-subtle">
        ...
        {isBatchMode ? (
          ...
          <span className="absolute left-2 top-2 rounded-md bg-black/70 px-1.5 py-1 text-[13px] font-medium text-white backdrop-blur-sm">{USE_LABELS[asset.materialUse] ?? '通用素材'}</span>
        )}
      ...
      <div className="p-3">
        <div>
          ...
          <p className="mt-1 line-clamp-1 text-[13px] text-text-tertiary">{asset.sourceLabel} · {asset.resolution}</p>
        </div>

        <div className="mt-2 flex min-h-5 items-center gap-1.5">
          {primaryTag ? <span className="truncate rounded-md bg-surface-subtle px-1.5 py-0.5 text-[13px] text-text-secondary">{primaryTag}</span> : null}
          {asset.category === 'derived_material' ? <span className="shrink-0 rounded-md bg-neutral-100 px-1.5 py-0.5 text-[13px] text-neutral-700">差异化版本</span> : <span className="ml-auto shrink-0 text-[13px] text-text-tertiary">原始素材</span>}
        </div>

        {mode === 'optimize' ? (
          <div className="mt-3 rounded-lg bg-neutral-100 p-2.5">
            <div className="flex items-center gap-1 text-[13px] font-semibold text-neutral-800"><WandSparkles size={11} />优化结果已准备</div>
            <p className="mt-1 line-clamp-2 text-[13px] leading-4 text-neutral-600">{optimizationHint}</p>
          </div>
        ) : null}

        {asset.status === 'reserved' && asset.usageRelation?.noteTitle ? (
          <div className="mt-3 rounded-lg bg-blue-50 px-2.5 py-2 text-[13px] leading-4 text-blue-700">已锁定给《{asset.usageRelation.noteTitle}》，不能被其他笔记选用。</div>
        ) : null}

        {hasFooterAction ? <div className="mt-3 flex items-center justify-end gap-2 border-t border-border-subtle pt-2.5">
          {mode === 'optimize' ? (
            <button type="button" onClick={event => { event.stopPropagation(); onOpenOptimization?.(asset); }} className="rounded-lg bg-btn-main px-2.5 py-1.5 text-[13px] font-medium text-white">查看优化结果</button>
          ) : null}
          {mode === 'history' && asset.status === 'used' ? (
            <button type="button" onClick={event => { event.stopPropagation(); onCreateVariant?.(asset); }} className="flex items-center gap-1 rounded-lg border border-border-default px-2.5 py-1.5 text-[13px] font-medium text-text-main hover:bg-hover-bg"><WandSparkles size={10} />生成差异化版本</button>
          ) : null}
        </div> : null}
      </div>
    </article>
