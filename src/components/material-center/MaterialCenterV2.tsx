      ...
      <aside className="flex h-full w-full max-w-[560px] flex-col bg-surface-1 shadow-2xl" onClick={event => event.stopPropagation()} aria-label="素材优化结果">
        <div className="flex items-start justify-between border-b border-border-default px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-tertiary">系统已基于真实原图准备结果，不改变产品和体验事实。</p>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="mb-2 text-[13px] font-medium text-text-secondary">原始素材</div>
            ...
            <div>
              <div className="mb-2 flex items-center justify-between text-[13px] font-medium text-text-secondary"><span>准备结果</span><span className="text-emerald-700">保持实拍</span></div>
          ...
          <section className="mt-5 rounded-xl border border-border-default p-4">
            <h3 className="text-[13px] font-semibold text-text-main">选择处理方式</h3>
            <div className="mt-3 grid grid-cols-3 gap-2">
              {options.map(option => (
                <button key={option} type="button" onClick={() => setSelectedOption(option)} className={`rounded-lg border px-3 py-2.5 text-[13px] font-medium transition-colors ${selectedOption === option ? 'border-neutral-900 bg-neutral-100 text-neutral-900' : 'border-border-default text-text-secondary hover:bg-hover-bg'}`}>
                  {option}
                </button>
              ))}
            </div>
            <div className="mt-3 rounded-lg bg-surface-subtle px-3 py-2.5 text-[13px] leading-5 text-text-secondary">
              {context === 'variant'
        ...
        <div className="flex items-center justify-between border-t border-border-default px-5 py-4">
          <span className="text-[13px] text-text-tertiary">只保存确认采用的版本</span>
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="rounded-lg border border-border-default px-4 py-2 text-[13px] font-medium text-text-secondary">取消</button>
            <button type="button" onClick={() => onSave(selectedOption)} className="rounded-lg bg-btn-main px-4 py-2 text-[13px] font-medium text-white">确认并保存新版本</button>
          </div>
        </div>
      </aside>
    </div>
      ...
      <aside className="flex h-full w-full max-w-[580px] flex-col bg-surface-1 shadow-2xl" onClick={event => event.stopPropagation()} aria-label="批量处理素材">
        <div className="flex items-start justify-between border-b border-border-default px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-tertiary">已选 {assets.length} 项素材，只需选择本次要完成的动作。</p>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="flex gap-2 overflow-hidden rounded-xl bg-surface-subtle p-2">
            ...
            {assets.length > 5 ? <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-white text-[13px] font-medium text-text-secondary">+{assets.length - 5}</div> : null}
          </div>
            ...
            {BATCH_ACTIONS.map(action => {
              ...
              return (
                <button key={action.id} type="button" onClick={() => setActiveAction(action.id)} className={`rounded-xl border p-3 text-left transition-colors ${active ? action.danger ? 'border-red-300 bg-red-50' : 'border-neutral-900 bg-neutral-100' : 'border-border-default hover:bg-hover-bg'}`}>
                  <div className={`flex items-center gap-2 text-[13px] font-semibold ${action.danger ? 'text-red-600' : 'text-text-main'}`}><Icon size={14} />{action.title}</div>
                  <p className="mt-1.5 text-[13px] leading-4 text-text-tertiary">{action.description}</p>
                </button>
              );
            })}
          </div>
          ...
          <section className="mt-4 rounded-xl border border-border-default p-4">
            {activeAction === 'watermark' ? (
              <div>
                <h3 className="text-[13px] font-semibold text-text-main">去水印规则</h3>
                <div className="mt-3 flex items-start gap-2 rounded-lg bg-surface-subtle p-3"><Check size={12} className="mt-0.5 shrink-0 text-emerald-700" /><div><div className="text-[13px] font-medium text-text-main">仅处理识别到水印的图片</div><p className="mt-1 text-[13px] leading-4 text-text-tertiary">固定角标优先使用裁切或遮罩；复杂水印生成候选结果后由你确认。未检出水印的图片不会生成副本。</p></div></div>
              </div>
            ) : null}
            {activeAction === 'resize' ? (
              <div>
                <h3 className="text-[13px] font-semibold text-text-main">目标尺寸</h3>
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {['3:4 · 1242×1656', '1:1 · 1080×1080', '原比例高清'].map(option => <button key={option} type="button" onClick={() => setTargetRatio(option)} className={`rounded-lg border px-2 py-2.5 text-[13px] ${targetRatio === option ? 'border-neutral-900 bg-neutral-100 font-medium text-neutral-900' : 'border-border-default text-text-secondary'}`}>{option}</button>)}
                </div>
                <p className="mt-3 text-[13px] leading-4 text-text-tertiary">默认保护主体和文字安全区；尺寸不足时生成延展候选，不直接拉伸原图。</p>
              </div>
            ) : null}
            {activeAction === 'tag' ? (
              <div>
                <h3 className="text-[13px] font-semibold text-text-main">选择手动标签</h3>
                <div className="mt-3 flex flex-wrap gap-2">{customTags.map(tagName => <button key={tagName} type="button" onClick={() => setTargetTag(tagName)} className={`rounded-lg px-2.5 py-1.5 text-[13px] ${targetTag === tagName ? 'bg-btn-main font-medium text-white' : 'bg-surface-subtle text-text-secondary'}`}>{tagName}</button>)}</div>
                <p className="mt-3 text-[13px] text-text-tertiary">手动标签用于操盘手归类；AI自动标签仍在后台参与搜索和智能匹配。</p>
              </div>
            ) : null}
            {activeAction === 'delete' ? (
              <div className="rounded-lg bg-red-50 p-3"><div className="text-[13px] font-semibold text-red-700">删除前需要再次确认</div><p className="mt-1 text-[13px] leading-4 text-red-600">已发布笔记中的使用关系和缩略记录会保留，但原始云端文件将不再出现在素材中心。</p></div>
            ) : null}
          </section>
        </div>

        <div className="flex items-center justify-between border-t border-border-default px-5 py-4">
          <span className="text-[13px] text-text-tertiary">处理结果不会覆盖原图</span>
          <div className="flex gap-2"><button type="button" onClick={onClose} className="rounded-lg border border-border-default px-4 py-2 text-[13px] text-text-secondary">取消</button><button type="button" onClick={submit} className={`rounded-lg px-4 py-2 text-[13px] font-medium text-white ${activeAction === 'delete' ? 'bg-red-600' : 'bg-btn-main'}`}>{submitLabel}</button></div>
        </div>
      </aside>
    </div>
  );
        ...
        <div className="workspace-header flex items-center justify-between gap-5">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-md border border-border-default bg-surface-subtle px-1.5 py-0.5 text-[13px] text-text-tertiary">{counts.available}项可用</span>
            </div>
        ...
        <div className="mx-auto max-w-[1500px] space-y-4">
          <section className="workspace-surface workspace-filterbar rounded-xl border border-border-default bg-surface-1 p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="ml-auto text-[13px] text-text-tertiary">{visibleAssets.length}项素材</span>
            </div>
            ...
            {showFilters ? (
              <div className="mt-3 grid grid-cols-3 gap-2 border-t border-border-subtle pt-3">
                <select value={selectedUse} onChange={event => setSelectedUse(event.target.value)} className="rounded-lg border border-border-default bg-surface-1 px-2.5 py-2 text-[13px] text-text-secondary"><option value="all">所有用途</option>{Object.entries(USE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
                <select value={selectedSource} onChange={event => setSelectedSource(event.target.value)} className="rounded-lg border border-border-default bg-surface-1 px-2.5 py-2 text-[13px] text-text-secondary"><option value="all">所有来源</option><option value="merchant">商家上传</option><option value="task_upload">任务回传</option><option value="koc">KOC回传</option><option value="ai_derived">衍生版本</option></select>
                <div className="flex gap-2">
          ...
          {isBatchMode && selectedCount > 0 ? (
            <div className="sticky bottom-3 z-20 mx-auto flex max-w-2xl items-center justify-between rounded-xl border border-border-strong bg-surface-1 px-4 py-3 shadow-xl">
              <div className="text-[13px] font-medium text-text-main">已选 {selectedCount} 项</div>
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={() => setNotice(`已开始打包下载${selectedCount}项素材。`)} className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[13px] text-text-secondary hover:bg-hover-bg"><Download size={11} />下载</button>
                <button type="button" onClick={() => setShowBatchDrawer(true)} className="flex items-center gap-1.5 rounded-lg bg-btn-main px-3 py-1.5 text-[13px] font-medium text-white"><WandSparkles size={11} />批量处理</button>
              </div>
            </div>
          ) : null}
        </div>
      ...
      {showTagModal ? (
        <div className="fixed inset-0 z-[140] flex items-center justify-center bg-black/30 p-4" onClick={() => setShowTagModal(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-surface-1 p-5 shadow-2xl" onClick={event => event.stopPropagation()}>
            ...
            <input autoFocus value={newTagName} onChange={event => setNewTagName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') addTag(); }} placeholder="例如：9月新品、青岛门店" className="mt-4 w-full rounded-lg border border-border-default px-3 py-2.5 text-[13px] outline-none focus:border-border-strong" />
            <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setShowTagModal(false)} className="rounded-lg px-3 py-2 text-[13px] text-text-secondary">取消</button><button type="button" onClick={addTag} disabled={!newTagName.trim()} className="rounded-lg bg-btn-main px-4 py-2 text-[13px] font-medium text-white disabled:opacity-40">确认</button></div>
          </div>
        </div>
      ) : null}
      ...
      {showDeleteConfirm ? (
        <div className="fixed inset-0 z-[140] flex items-center justify-center bg-black/30 p-4" onClick={() => setShowDeleteConfirm(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-surface-1 p-5 shadow-2xl" onClick={event => event.stopPropagation()}>
            ...
            <p className="mt-1.5 text-[13px] leading-5 text-text-secondary">将删除 {selectedCount} 项素材。已发布或已绑定的使用记录仍会保留在对应笔记中。</p>
