           ...
           <div className="px-8 pt-6 pb-5 border-b border-neutral-100 flex flex-col space-y-5 bg-neutral-50/30">
              ...
              <div className="flex items-center gap-4">
                 ...
                 <div className="flex items-center p-1 bg-neutral-100/60 rounded-[10px]">
                    <button onClick={() => setActiveTab('all')} className={`px-3 py-1.5 rounded-lg text-[12px] font-black transition-all ${activeTab === 'all' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}>全部</button>
                    <button onClick={() => setActiveTab('active')} className={`px-3 py-1.5 rounded-lg text-[12px] font-black transition-all ${activeTab === 'active' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}>服务中</button>
                    <button onClick={() => setActiveTab('pending')} className={`px-3 py-1.5 rounded-lg text-[12px] font-black transition-all ${activeTab === 'pending' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}>待冷启</button>
                 </div>
              </div>
           </div>
                    ...
                    return (
                       ...
                       >
                          ...
                          <div className="col-span-4 lg:col-span-5 flex items-center gap-3 pr-4 min-w-0">
                             <div 
                               className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-[14px] shadow-sm shrink-0 uppercase transition-transform group-hover:scale-105"
                             ...
                             <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2 mb-0.5">
                                   <h3 className={`text-[14px] font-black tracking-tight truncate ${isActive ? 'text-primary-600' : 'text-neutral-900'}`}>{proj.name}</h3>
                                ...
                                <div className="flex items-center gap-1 flex-wrap">
                                   {proj.tags?.map((tag: string) => (
                                      <span key={tag} className="text-[10px] font-bold px-1.5 py-0.5 bg-neutral-100 text-neutral-500 rounded truncate max-w-[60px]">
                                         {tag}
                                      </span>
                                   ))}
                                </div>
                          ...
                          <div className="col-span-4 lg:col-span-3 flex items-center gap-3 min-w-0">
                             <div className="flex flex-col xl:flex-row items-center gap-1 bg-neutral-50 px-2 py-1 rounded-md border border-neutral-100 shrink-0">
                                <div className="flex items-center gap-1">
                                   ...
                                   <span className="text-[12px] font-black text-neutral-700">{proj.stats?.pendingContent || 0}</span>
                                </div>
                                <span className="text-[9px] text-neutral-400 font-bold">篇待发</span>
                             </div>
                             <div className="flex flex-col xl:flex-row items-center gap-1 bg-neutral-50 px-2 py-1 rounded-md border border-neutral-100 shrink-0">
                                <div className="flex items-center gap-1">
                                   <AlertCircle size={10} className="text-warning-500" />
                                   <span className="text-[12px] font-black text-neutral-700">{proj.stats?.pendingLeads || 0}</span>
                                </div>
                                <span className="text-[9px] text-neutral-400 font-bold">待回</span>
                             </div>
                          </div>
                             ...
                             <div className="flex items-center gap-2">
                               {proj.stats?.profileCompleteness === 100 ? (
                                  <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-50 rounded-md shrink-0">
                                     ...
                                     <span className="text-[11px] font-black text-emerald-600">已完善</span>
                                  </div>
                               ) : (
                                  <div className="flex items-center gap-2 shrink-0">
                                     ...
                                     <span className="text-[11px] font-black text-neutral-500">{proj.stats?.profileCompleteness || 0}%</span>
                                  </div>
                               )}
                             </div>
                          </div>
