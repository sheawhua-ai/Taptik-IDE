...
export const MerchantManagement = () => {
 ...
 className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-[13px] hover:bg-primary-500 transition-colors shadow-sm"
 >
 <Plus size={16} />
 新增商家
 </button>
 ...
 className="w-full h-10 pl-10 pr-4 bg-white border border-slate-200 focus:border-primary-500 outline-none rounded-lg text-[13px] transition-all shadow-sm"
 />
 </div>
 </div>

 <div className="flex-1 bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
 <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-slate-50/80 border-b border-slate-100 text-[12px] text-slate-400 uppercase tracking-widest sticky top-0">
 ...
 <h4 className="text-[14px] font-semibold text-slate-900 truncate">{merchant.name}</h4>
 <p className="text-[12px] text-slate-400 font-medium truncate mt-0.5">创建于 {merchant.createdAt}</p>
 </div>
 </div>
 <div className="col-span-3 flex flex-col min-w-0">
 <div className="flex items-center gap-1.5 text-[13px] text-slate-700 truncate">
 ...
 <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-50 text-green-600 text-[11px] ">
 <ShieldCheck size={12} /> 正常运营
 </span>
 ) : (
 <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-warning-50 text-warning-600 text-[11px] ">
 冷启建设中
 </span>
 )}
 </div>
