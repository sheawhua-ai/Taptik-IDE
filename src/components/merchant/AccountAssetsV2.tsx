          ...
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-border-default bg-surface shadow-2xl" onClick={event => event.stopPropagation()}>
            ...
            <div className="bg-canvas p-5">
              <div className="overflow-hidden rounded-xl border border-border-default bg-white shadow-sm">
                <div className="flex items-center gap-2 border-b border-border-default bg-surface-subtle px-3 py-2"><span className="h-2.5 w-2.5 rounded-full bg-rose-400" /><span className="h-2.5 w-2.5 rounded-full bg-amber-300" /><span className="h-2.5 w-2.5 rounded-full bg-emerald-400" /><div className="ml-2 flex flex-1 items-center justify-center rounded-md border border-border-default bg-white py-1 text-[13px] text-text-tertiary">https://creator.xiaohongshu.com</div></div>
                {authorizationStep === "login" && <div className="grid min-h-[330px] md:grid-cols-[1.15fr_1fr]"><div className="flex flex-col justify-center bg-gradient-to-br from-rose-50 to-white p-8"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#ff2442] text-[15px] font-bold text-white">薯</div><h4 className="mt-4 text-[18px] font-semibold text-text-primary">小红书创作服务平台</h4><p className="mt-2 text-[13px] leading-6 text-text-secondary">登录后授权 Taptik 获取当前账号的基础资料和创作数据，用于账号矩阵、发布日历与复盘。</p><div className="mt-4 flex items-center gap-2 text-[13px] text-text-tertiary"><ShieldCheck size={14} className="text-emerald-600" />不在 Taptik 保存登录密码</div></div><div className="flex flex-col justify-center p-8"><div className="text-[13px] font-semibold text-text-primary">使用小红书账号登录</div><p className="mt-1 text-[13px] text-text-tertiary">将在创作服务平台完成登录与授权。</p><button onClick={() => { window.open("https://creator.xiaohongshu.com", "_blank", "noopener,noreferrer"); setAuthorizationStep("waiting"); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-[#ff2442] px-4 py-2.5 text-[13px] font-semibold text-white">打开登录页面<ExternalLink size={13} /></button><div className="mt-3 text-center text-[13px] text-text-tertiary">完成登录后，本窗口将获取账号资料</div></div></div>}
                {authorizationStep === "waiting" && <div className="flex min-h-[330px] flex-col items-center justify-center p-8 text-center"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-50"><RefreshCw size={20} className="animate-spin text-blue-600" /></div><h4 className="mt-4 text-[14px] font-semibold text-text-primary">等待小红书登录授权</h4><p className="mt-2 max-w-sm text-[13px] leading-5 text-text-tertiary">请在刚打开的创作服务平台完成登录。授权回调后将自动读取头像、昵称和小红书号。</p><button onClick={() => setAuthorizationStep("profile")} className="mt-5 rounded-lg border border-border-default bg-white px-4 py-2 text-[13px] font-medium text-text-primary">已完成登录，获取账号资料</button></div>}
                {authorizationStep === "profile" && <div className="min-h-[330px] p-7"><div className="flex items-center gap-2 text-[13px] font-semibold text-emerald-700"><CheckCircle2 size={15} />已获取小红书账号资料</div><div className="mt-4 flex items-center gap-3 rounded-xl bg-surface-subtle p-4"><img src="https://images.unsplash.com/photo-1517849845537-4d257902454a?w=96&h=96&fit=crop" alt="" className="h-14 w-14 rounded-full object-cover" /><div><div className="text-[14px] font-semibold text-text-primary">陆家嘴萌宠顾问</div><div className="mt-1 text-[13px] text-text-tertiary">小红书号：xhs_668821039</div><div className="mt-1 text-[13px] text-emerald-700">头像、昵称来自小红书接口</div></div></div><label className="mt-5 block"><span className="text-[13px] font-medium text-text-secondary">加入后的账号关系</span><select value={newRelation} onChange={event => setNewRelation(event.target.value as AccountRelation)} className="mt-1.5 w-full rounded-lg border border-border-default bg-white px-3 py-2.5 text-[13px] outline-none"><option>自有品牌号</option><option>员工KOS</option><option>协作KOC</option></select></label><div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-[13px] leading-5 text-blue-900">加入后进入“运营配置”，继续设置账号角色、人设、专用手机、发布员工和发布指令。</div></div>}
              </div>
            </div>
            <div className="flex items-center justify-between border-t border-border-default px-5 py-4"><span className="text-[13px] text-text-tertiary">一台手机仅绑定一个账号；员工可负责多个账号。</span><div className="flex gap-2"><button onClick={() => setShowAddModal(false)} className="rounded-lg px-3 py-2 text-[13px] text-text-secondary">取消</button>{authorizationStep === "profile" && <button onClick={addAccount} className="rounded-lg bg-action-primary px-4 py-2 text-[13px] font-semibold text-white">确认加入账号</button>}</div></div>
          </div>
        </div>
      )}

      {feedback && <div className="fixed bottom-6 left-1/2 z-[80] -translate-x-1/2 rounded-lg bg-neutral-950 px-4 py-2.5 text-[13px] text-white shadow-xl">{feedback}</div>}
    </div>
  );
};

function EmptyState({ icon: Icon, title, detail }: { icon: React.ElementType; title: string; detail: string }) {
  return <div className="rounded-xl border border-dashed border-border-strong bg-surface px-4 py-14 text-center"><Icon size={24} className="mx-auto text-text-tertiary" /><div className="mt-2 text-[13px] font-medium text-text-primary">{title}</div><div className="mt-1 text-[13px] text-text-tertiary">{detail}</div></div>;
}
...
function DataScopeNotice({ profile, scope }: { profile: AccountProfile; scope: string }) {
  ...
  return <div className="flex items-start justify-between gap-3 rounded-xl border border-border-default bg-surface px-4 py-3"><div><div className="text-[13px] font-semibold text-text-primary">{scope}</div><div className="mt-0.5 text-[13px] text-text-tertiary">数据来自账号后台爬虫采集快照；未取得的字段显示“未获取”，不用0填充。</div></div><span className={`shrink-0 rounded-md border px-2 py-1 text-[13px] ${covered ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-800"}`}>{covered ? "已纳入采集" : "当前未覆盖"}</span></div>;
}

function InfoCard({ label, value, icon: Icon }: { label: string; value: string; icon: React.ElementType }) {
  return <div className="rounded-xl border border-border-default bg-surface p-4"><div className="flex items-center gap-1.5 text-[13px] text-text-tertiary"><Icon size={13} />{label}</div><div className="mt-2 text-[13px] font-semibold text-text-primary">{value}</div></div>;
}

function ConfigField({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return <label className="block"><span className="flex items-center justify-between gap-2 text-[13px] font-medium text-text-secondary"><span>{label}</span>{hint && <span className="font-normal text-text-tertiary">{hint}</span>}</span><span className="mt-1.5 block">{children}</span></label>;
}
