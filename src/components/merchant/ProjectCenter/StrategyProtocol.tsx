  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12 relative">
      <div className="flex justify-end absolute -top-12 right-0">
         <button onClick={() => setDrawerOpen("capability")} className="px-4 py-2 bg-btn-main text-white rounded-lg text-[13px] font-bold hover:bg-btn-main-hover transition-colors flex items-center gap-2">
           <Cpu size={16} /> 查看能力方案
         </button>
      </div>

      <div className="bg-surface-1 rounded-xl p-6 border border-border-default ">
        <h2 className="text-[16px] font-bold mb-6 flex items-center gap-2">
          <CheckCircle2 size={18} className="text-brand-logo" /> 已确认事实
        </h2>
        <div className="space-y-4">
          {[
            ...
            <div key={item.id} className="p-4 bg-surface-2 rounded-xl border border-border-default flex justify-between items-center">
               ...
               <button onClick={() => setDrawerOpen("source")} className="text-[13px] text-brand-logo font-bold hover:underline flex items-center gap-1">
      ...
      <div className="bg-surface-1 rounded-xl p-6 border border-border-default ">
        <h2 className="text-[16px] font-bold mb-6 flex items-center gap-2">
          <FlaskConical size={18} className="text-brand-logo" /> 待验证假设
        </h2>
        <div className="space-y-4">
      ...
      <div className="bg-surface-1 rounded-xl p-6 border border-border-default ">
        <h2 className="text-[16px] font-bold mb-6 flex items-center gap-2">
          <ShieldAlert size={18} className="text-danger" /> 执行边界
        </h2>
        <div className="grid grid-cols-2 gap-6">
           <div className="space-y-3">
             <div className="text-[14px] font-bold text-text-main border-b pb-2">禁止表达</div>
           ...
           <div className="space-y-3">
             <div className="text-[14px] font-bold text-text-main border-b pb-2">限制条件</div>
             <ul className="list-disc pl-5 text-[13px] text-text-secondary space-y-1">
      ...
      <AnimatePresence>
        {drawerOpen === "capability" && (
           <div className="fixed inset-0 z-50 flex justify-end">
             <div className="absolute inset-0 bg-btn-main/20 backdrop-blur-sm" onClick={() => setDrawerOpen(null)} />
             <motion.div 
               initial={{ x: "100%" }}
               animate={{ x: 0 }}
               exit={{ x: "100%" }}
               className="w-[600px] bg-surface-1 h-full shadow-2xl flex flex-col relative z-10"
             >
               <div className="p-6 border-b border-border-default flex justify-between items-center bg-[#fcfcfc]">
                 <h2 className="text-[18px] font-bold">能力调用方案</h2>
                 <button onClick={() => setDrawerOpen(null)} className="p-2 text-text-tertiary hover:text-text-main rounded-lg hover:bg-hover-bg">
                   <X size={20} />
                 </button>
               </div>
