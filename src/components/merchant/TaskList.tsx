  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-[20px] font-black text-neutral-900 tracking-tight">素材任务队列</h2>
          <p className="text-[12px] text-neutral-400 font-bold uppercase tracking-wider mt-1">Batch Processing & Asset Tasks</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {MOCK_TASKS.map((task) => (
          <div key={task.id} className="bg-white rounded-[32px] border border-neutral-200 p-6 shadow-sm hover:shadow-xl transition-all group">
            ...
            <h3 className="text-[16px] font-black text-neutral-900 mb-2">{task.name}</h3>
            <p className="text-[11px] text-neutral-400 font-bold uppercase tracking-widest mb-6">Task ID: {task.id}</p>
            ...
            <div className="space-y-4">
               ...
               <div className="flex items-center justify-between pt-4 border-t border-neutral-50">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-[11px] font-bold text-neutral-400">{task.createdAt}</span>
                  </div>
                  <div className="flex items-center gap-2">
                     {task.status === 'running' ? (
                       ...
                       <div className="flex items-center gap-1 text-success-500 text-[12px] font-black"><CheckCircle2 size={14}/> 已完成</div>
                     )}
                  </div>
               </div>
            </div>
