...
export const AssetManager: React.FC<{ embedded?: boolean }> = ({ embedded }) => {
 ...
 <div>
 <h2 className="text-[20px] font-semibold text-neutral-900 tracking-tight">素材管理中心</h2>
 <p className="text-[12px] text-neutral-400 uppercase tracking-wider mt-1">素材储存库与媒体库</p>
 </div>
 ...
 <button className="px-5 py-2 bg-neutral-900 text-white rounded-xl text-[12px] shadow-lg shadow-neutral-200 flex items-center gap-2 hover:bg-primary-500 transition-all">
 <Plus size={16}/> 上传素材
 </button>
 </div>
 <span className="text-[11px] text-neutral-400 uppercase tracking-widest">全局资产库</span>
 </div>
 )}
 ...
 className="w-full pl-12 pr-4 py-3 bg-neutral-0 border border-neutral-200 rounded-[20px] text-[14px] focus:outline-none focus:ring-4 focus:ring-primary-500/5 transition-all"
 />
 </div>
 ...
 className={`px-4 py-2 rounded-xl text-[12px] capitalize transition-all ${selectedType === type ? 'bg-neutral-900 text-white shadow-lg shadow-neutral-200' : 'bg-neutral-0 border border-neutral-200 text-neutral-500 hover:bg-neutral-50'}`}
 >
 ...
 <div className="p-5">
 <h3 className="text-[14px] font-semibold text-neutral-800 truncate mb-1">{asset.name}</h3>
 <div className="flex items-center justify-between">
 <span className="text-[11px] text-neutral-400">{asset.size}</span>
 <div className="flex gap-1">
 {asset.tags.slice(0, 2).map(tag => (
 <span key={tag} className="text-[9px] text-primary-500 bg-primary-50 px-1.5 py-0.5 rounded">#{tag}</span>
 ))}
 </div>
 </div>
 </div>
 ...
 <td className="px-8 py-4">
 <span className="text-[14px] text-neutral-800">{asset.name}</span>
 </td>
 <td className="px-8 py-4 text-[13px] text-neutral-500">{asset.type === 'image' ? '图片' : asset.type === 'video' ? '视频' : asset.type === 'audio' ? '音频' : '文档'}</td>
 ...
 <td className="px-8 py-4 text-[13px] text-neutral-400">{asset.createdAt}</td>
