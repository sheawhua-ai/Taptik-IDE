...
export function ProposalDiffChatCard({
  ...
  return (
    <div className="mt-3 bg-surface-1 border border-border-default rounded-xl overflow-hidden shadow-sm text-[12.5px] space-y-3 p-4">
      {/* Header */}
      <div className="flex items-center justify-between pb-2 border-b border-border-subtle">
        ...
        <span className="text-[11px] px-2 py-0.5 bg-surface-subtle border border-border-default rounded font-medium text-text-secondary">
          影响 {proposal.impactScope.affectedNotesCount} 篇笔记
        </span>
      </div>

      {/* AI Interpretation */}
      <div className="p-2.5 bg-surface-subtle border border-border-subtle rounded-lg text-[12px] text-text-primary leading-relaxed">
      ...
      <div className="space-y-2">
        <div className="text-[11.5px] font-semibold text-text-tertiary">
          变更前后对比：
        </div>
        <div className="space-y-1.5">
          {proposal.diffSummary.map((item, idx) => (
            <div key={idx} className="p-2.5 bg-white border border-border-subtle rounded-lg space-y-1">
              ...
              <div className="text-[11.5px] text-rose-800 line-through">
                调整前：{item.before}
              </div>
              <div className="text-[12px] text-emerald-800 font-medium flex items-start gap-1">
                <span>调整后：</span>
      ...
      {/* Impact Scope */}
      <div className="p-2.5 bg-surface-subtle border border-border-subtle rounded-lg text-[11.5px] space-y-1 text-text-secondary">
      ...
      <div className="pt-2 border-t border-border-subtle flex items-center justify-end gap-2">
        {onReject && (
          <button
            type="button"
            onClick={onReject}
            className="px-3 py-1.5 bg-surface-1 hover:bg-surface-hover border border-border-default rounded-lg text-[12px] text-text-secondary font-medium transition-colors"
          >
            放弃修改
          </button>
        )}
        ...
        <button
          ...
          className="px-4 py-1.5 bg-action-primary hover:bg-action-primary-hover text-white rounded-lg text-[12px] font-semibold transition-colors flex items-center gap-1.5 shadow-sm"
        >
          <Check size={13} />
