        ...
        <div className="p-6 overflow-y-auto space-y-5 text-[13px]">
          ...
          <div className="p-3.5 bg-surface-subtle border border-border-default rounded-lg space-y-2">
            <div className="flex items-start gap-2">
              <span className="text-[11px] px-1.5 py-0.5 bg-neutral-200 text-neutral-800 rounded font-medium shrink-0">
                用户要求
              </span>
            ...
            <div className="flex items-start gap-2 pt-1 border-t border-border-subtle">
              <span className="text-[11px] px-1.5 py-0.5 bg-blue-50 text-blue-800 border border-blue-200 rounded font-medium shrink-0">
                AI 理解
              </span>
          ...
          <div>
            <div className="text-[12.5px] font-semibold text-text-primary mb-2 flex items-center gap-1.5">
              <span>修改前后差异对比</span>
            </div>
            ...
            <div className="border border-border-default rounded-lg divide-y divide-border-subtle overflow-hidden">
              {proposal.diffSummary.map((diff, idx) => (
                <div key={idx} className="p-3 bg-surface-1">
                  <div className="text-[11.5px] font-medium text-text-tertiary mb-1.5">
                    {diff.moduleName}
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[12px]">
                    <div className="p-2 bg-rose-50/50 border border-rose-200/70 rounded text-rose-900 line-through decoration-rose-400">
                      <span className="text-[10px] block text-rose-700 font-semibold mb-0.5">修改前</span>
                      {diff.before}
                    </div>
                    <div className="p-2 bg-emerald-50/60 border border-emerald-200/80 rounded text-emerald-950">
                      <span className="text-[10px] block text-emerald-700 font-semibold mb-0.5">修改后 (提案)</span>
                      {diff.after}
                    </div>
                  </div>
                </div>
          ...
          {/* Impact Scope */}
          <div>
            <div className="text-[12.5px] font-semibold text-text-primary mb-2">
              连带影响与任务变动评估
            </div>

            <div className="p-3.5 bg-surface-1 border border-border-default rounded-lg space-y-3 text-[12px]">
              ...
              <div className="space-y-1.5 pt-1">
                ...
                {proposal.impactScope.taskChanges.added.length > 0 && (
                  <div className="flex items-start gap-1.5 text-emerald-800 text-[11.5px]">
                    <span className="px-1 bg-emerald-100 rounded text-[10px] font-bold">+ 新增</span>
                    <span>{proposal.impactScope.taskChanges.added.join('；')}</span>
                  </div>
                )}
                {proposal.impactScope.taskChanges.removed.length > 0 && (
                  <div className="flex items-start gap-1.5 text-rose-800 text-[11.5px]">
                    <span className="px-1 bg-rose-100 rounded text-[10px] font-bold">- 取消</span>
                    <span>{proposal.impactScope.taskChanges.removed.join('；')}</span>
                  </div>
                )}
                {proposal.impactScope.taskChanges.modified.length > 0 && (
                  <div className="flex items-start gap-1.5 text-blue-800 text-[11.5px]">
                    <span className="px-1 bg-blue-100 rounded text-[10px] font-bold">~ 变更</span>
              ...
              {/* Fact conflict check */}
              <div className="pt-2 border-t border-border-subtle flex items-center justify-between text-[11.5px]">
        ...
        <div className="px-6 py-3.5 border-t border-border-default bg-surface-subtle flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onReject}
              className="px-3.5 py-1.5 bg-surface-1 hover:bg-surface-hover border border-border-default rounded-lg text-[12.5px] text-text-secondary font-medium transition-colors"
            >
              放弃修改
            </button>
            <button
              ...
              className="px-4 py-1.5 bg-action-primary hover:bg-action-primary-hover text-white rounded-lg text-[12.5px] font-medium transition-colors flex items-center gap-1.5 shadow-sm"
            >
              <Check size={14} />
