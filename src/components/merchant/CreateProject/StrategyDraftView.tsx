        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              {isEditingName ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="px-2 py-1 bg-action-primary text-white rounded text-[11.5px] font-medium"
                  >
                    保存
                  </button>
                </div>
              ...
              <span className="text-[11px] px-2 py-0.5 rounded font-medium bg-blue-50 text-blue-800 border border-blue-200">
                待确认打法
              </span>
            </div>
        ...
        <div className="bg-surface-1 rounded-xl border border-border-default shadow-sm divide-y divide-border-default overflow-hidden">
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded bg-neutral-100 text-text-primary text-[11px] font-bold flex items-center justify-center">
                  1
                </span>
            ...
            <div className="bg-surface-subtle border border-border-subtle rounded-lg p-4 space-y-3 text-[13px]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <span className="text-text-tertiary text-[12px] block mb-0.5">推广主体 / 核心产品</span>
                  <span className="font-semibold text-text-primary">
                    {draft.promotionTarget.targetName}
                  </span>
                  <span className="ml-2 text-[11.5px] text-text-tertiary">({draft.promotionTarget.targetCategory})</span>
                </div>
                <div>
                  <span className="text-text-tertiary text-[12px] block mb-0.5">目标人群与核心场景</span>
              ...
              <div className="pt-2 border-t border-border-subtle">
                <span className="text-[11.5px] font-medium text-text-tertiary block mb-1.5">已确认事实依据 (系统来源可追溯):</span>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[12px]">
                  {draft.promotionTarget.confirmedFacts.map((fact, fIdx) => (
                    <div key={fIdx} className="p-2.5 bg-white border border-border-subtle rounded-lg">
                      <div className="text-[10px] text-emerald-800 font-semibold mb-0.5">{fact.source}</div>
                      <div className="font-medium text-text-primary text-[11.5px]">{fact.label}</div>
                      <div className="text-[11px] text-text-tertiary mt-0.5">{fact.detail}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {draft.promotionTarget.unconfirmedGaps.length > 0 && (
                <div className="pt-2 text-[11.5px] text-amber-900 bg-amber-50/70 p-2.5 rounded-lg border border-amber-200/80 flex items-center gap-2">
          ...
          <div className="p-6">
            ...
            <div className="space-y-4 text-[13px]">
              ...
              <div>
                <span className="text-[12px] font-medium text-text-tertiary block mb-2">本周期可观察的验证信号:</span>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {draft.coreGoalAndVerification.observableSignals.map((signal, sIdx) => (
                    <div key={sIdx} className="p-3 bg-surface-subtle border border-border-subtle rounded-lg text-[12px]">
                      <div className="text-text-tertiary text-[10.5px] font-semibold mb-1">
                        验证信号 {sIdx + 1}
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1 text-[12px]">
                <div className="p-3 bg-emerald-50/40 border border-emerald-200/60 rounded-lg">
                  <span className="text-[10.5px] text-emerald-800 font-bold block mb-0.5">如何判断继续 (Success)</span>
                ...
                <div className="p-3 bg-blue-50/40 border border-blue-200/60 rounded-lg">
                  <span className="text-[10.5px] text-blue-800 font-bold block mb-0.5">何时需调整 (Adjustment)</span>
                ...
                <div className="p-3 bg-rose-50/40 border border-rose-200/60 rounded-lg">
                  <span className="text-[10.5px] text-rose-800 font-bold block mb-0.5">何时需终止 (Stop Rule)</span>
            ...
            <div className="space-y-3 text-[12.5px]">
              ...
              {draft.accountAndContentAssignment.brandAccounts.map((acc) => (
                <div key={acc.id} className="p-3.5 bg-surface-subtle border border-border-subtle rounded-lg flex items-center justify-between">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] px-1.5 py-0.2 bg-neutral-200 text-neutral-800 rounded font-semibold">品牌主号</span>
                      <span className="font-semibold text-text-primary">{acc.name}</span>
                      <span className="text-[11.5px] text-text-tertiary">({acc.fans} 粉丝)</span>
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[11px] text-text-tertiary">{acc.frequency} · {acc.timeWindow}</div>
                  </div>
                </div>
              ))}
              ...
              <div className="p-3.5 bg-surface-subtle border border-border-subtle rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] px-1.5 py-0.2 bg-blue-100 text-blue-900 rounded font-semibold">店长号/KOS矩阵</span>
              ...
              {draft.accountAndContentAssignment.kocParticipants.enabled && (
                <div className="p-3.5 bg-surface-subtle border border-border-subtle rounded-lg space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] px-1.5 py-0.2 bg-amber-100 text-amber-900 rounded font-semibold">真实消费者KOC</span>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[12.5px]">
              ...
              <div className="p-4 bg-amber-50/30 border border-amber-200/70 rounded-lg space-y-2.5">
                ...
                <div className="space-y-2 text-[12px] text-amber-950">
          ...
          <div className="p-6">
            ...
            <div className="space-y-3 text-[12px]">
              ...
              <div className="space-y-1.5">
                <span className="text-text-tertiary font-medium text-[11px]">已确认事实清单：</span>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  {draft.hypothesesAndBasis.confirmedFacts.map((cf) => (
                    <div key={cf.id} className="p-2.5 bg-surface-subtle border border-border-subtle rounded-lg">
                      <div className="text-[10px] text-emerald-800 font-semibold mb-0.5">{cf.source}</div>
              ...
              <div className="space-y-1.5 pt-2">
                <span className="text-text-tertiary font-medium text-[11px]">AI 暂定假设 (非平台硬性规则，待执行数据验证)：</span>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  {draft.hypothesesAndBasis.pendingHypotheses.map((ph) => (
                    <div key={ph.id} className="p-2.5 bg-amber-50/50 border border-amber-200/70 rounded-lg">
                      <div className="text-[10px] text-amber-800 font-bold mb-0.5">AI 暂定假设</div>
                      <div className="text-amber-950 font-medium">{ph.text}</div>
                      <div className="text-[10.5px] text-amber-800/80 mt-1">{ph.basis}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="px-8 py-3 bg-surface-1 border-t border-border-default shrink-0 flex items-center gap-3">
        <form onSubmit={handleSendModifyCommand} className="flex-1 flex items-center gap-2">
          <input
            ...
            className="flex-1 h-9 px-3.5 bg-surface-subtle border border-border-default rounded-lg text-[12.5px] text-text-primary placeholder:text-text-tertiary outline-none focus:border-border-strong focus:bg-white transition-all"
          />
          <button
            type="submit"
            disabled={!naturalCommand.trim()}
            className="h-9 px-4 bg-action-primary hover:bg-action-primary-hover disabled:bg-neutral-200 disabled:text-neutral-400 text-white rounded-lg text-[12.5px] font-medium transition-colors flex items-center gap-1.5 shrink-0"
          >
            <Sparkles size={13} />
            <span>生成修改提案</span>
          </button>
        ...
        <button
          type="button"
          onClick={onConfirmAndCreate}
          className="h-9 px-5 bg-action-primary hover:bg-action-primary-hover text-white rounded-lg text-[12.5px] font-semibold transition-colors flex items-center gap-1.5 shadow-sm shrink-0"
        >
          <Check size={14} />
          <span>确认打法并创建项目</span>
        </button>
