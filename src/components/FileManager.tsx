...
export const FileManager: React.FC<FileManagerProps> = ({ 
 ...
 <h2 className="text-[15px] font-semibold tracking-widest text-neutral-900">知识库</h2>
 </div>
 
 <div className="flex bg-neutral-100 p-1 rounded-xl">
 <button 
 onClick={() => setActiveContext('m1')}
 className={`px-5 py-1.5 rounded-lg text-[12px] transition-all ${activeContext === 'm1' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-400 hover:text-neutral-600'}`}
 >
 项目专属知识
 </button>
 <button 
 onClick={() => setActiveContext('operator')}
 className={`px-5 py-1.5 rounded-lg text-[12px] transition-all ${activeContext === 'operator' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-400 hover:text-neutral-600'}`}
 >
 操盘手空间
 </button>
 </div>
 ...
 <h2 className="text-[16px] font-semibold tracking-tight text-neutral-900 flex items-center gap-2">
 ...
 <span className="px-2 py-0.5 bg-red-50 text-red-600 rounded text-[10px] uppercase tracking-widest border border-red-100 flex items-center gap-1">异动分析</span>
 ) : (
 <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded text-[10px] uppercase tracking-widest border border-emerald-100 flex items-center gap-1">已向量化</span>
 )}
 </div>
 <div className="flex gap-2">
 <button className="px-5 py-2 bg-neutral-900 text-white rounded-xl text-[12px] shadow-lg hover:bg-primary-500 transition-all flex items-center gap-2">
 ...
 <div>
 <h3 className="text-[14px] font-semibold text-red-900 mb-1">竞品新品视觉异动</h3>
 ...
 <span className="text-[12px] text-neutral-500">源采集图像片段</span>
 <span className="text-[10px] text-neutral-300 uppercase tracking-widest">Image Source</span>
 </div>
 ...
 <span className="text-[12px] text-primary-600 flex items-center gap-1.5"><Brain size={14} /> AI 结构化对比分析</span>
 </div>
 <div className="p-5 text-[13px] leading-relaxed text-neutral-700 font-medium space-y-4">
 <div>
 <div className=" text-neutral-900 mb-1">设计元素比对：</div>
 <p>竞品大量使用了低保真复古滤镜及中式印章元素。建议我们在应对策略中，强化自身的“现代自然”属性，以冷色调或清新亮色进行区隔排他。</p>
 </div>
 <div>
 <div className=" text-neutral-900 mb-1">模型建议动作：</div>
 ...
 <div>
 <h1 className="text-3xl font-semibold text-neutral-900 tracking-tight mb-2">{currentData.name}</h1>
 <p className="text-neutral-500 ">{currentData.desc}</p>
 </div>
 </div>
 ...
 <input placeholder="输入网址、文本片段，或拖拽文件到这里，AI将自动归类到相应分组..." className="flex-1 bg-transparent text-[13px] focus:outline-none text-neutral-800 placeholder:text-neutral-400" />
 <button className="h-11 px-6 bg-neutral-900 text-white rounded-xl text-[12px] shrink-0 hover:bg-primary-500 transition-colors shadow-md flex items-center gap-2">
 <Plus size={16} /> AI 一键收录
 </button>
 </div>
 <p className="text-[11px] text-neutral-400 mt-4 relative z-10 flex text-center items-center gap-2">
 ...
 <h2 className="text-[14px] font-semibold text-neutral-900 flex items-center gap-2">
 ...
 <div className="absolute top-4 right-4 text-[10px] text-blue-600 flex items-center gap-1.5 bg-blue-50/80 px-2 py-0.5 rounded-md border border-blue-100/50">
 <RefreshCw size={10} className="animate-spin-slow" /> 联网订阅
 </div>
 )}
 {item.isAlert && (
 <div className="absolute top-4 right-4 text-[10px] text-red-600 flex items-center gap-1.5 bg-red-50 px-2 py-0.5 rounded-md border border-red-200">
 ...
 <h4 className="text-[14px] font-semibold text-neutral-900 truncate group-hover:text-primary-600 transition-colors">{item.title}</h4>
 <div className="flex items-center gap-2 mt-1">
 <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
 <p className="text-[11px] text-neutral-400 truncate">来源: {item.source}</p>
 </div>
 </div>
 </div>
 ...
 <span key={t} className="text-[10px] bg-neutral-100/80 text-neutral-500 px-2 py-0.5 rounded-md uppercase tracking-tight group-hover:bg-primary-50 group-hover:text-primary-600 transition-colors">#{t}</span>
 ))}
 </div>
 ...
 <h3 className="text-[14px] font-semibold mb-1 text-neutral-900 group-hover:text-primary-700">添加卡片式文件夹</h3>
 <p className="text-[11px] ">由 AI 基于主题和模态自动维护的新分类集</p>
 </div>
