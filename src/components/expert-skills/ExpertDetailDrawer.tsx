...
export const ExpertDetailDrawer: React.FC<ExpertDetailDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-btn-main/40 backdrop-blur-xs transition-opacity" onClick={onClose} />

      {/* Drawer Container */}
      <div className="relative w-full max-w-2xl bg-surface-1 h-full shadow-2xl flex flex-col z-10 animate-in slide-in-from-right duration-200">
        {/* Header */}
        <div className="p-6 border-b border-border-default flex items-center justify-between bg-page-bg">
          ...
          <button onClick={onClose} className="p-2 text-text-tertiary hover:text-text-main hover:bg-hover-bg rounded-xl transition-all">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-[13px]">
          ...
          <div className="space-y-2 border-t border-border-default pt-4">
            <h3 className="font-extrabold text-text-main text-[14px]">已绑定技能</h3>
            <div className="space-y-2">
              {expert.boundSkills?.map(sk => (
                <div key={sk.id} className="p-3 bg-page-bg rounded-xl border border-border-default/80 flex items-center justify-between">
                  <div className="flex items-center gap-2">
          ...
          {/* Recent Task Logs */}
          <div className="space-y-2 border-t border-border-default pt-4">
            <h3 className="font-extrabold text-text-main text-[14px] flex items-center gap-2">
        ...
        <div className="p-4 border-t border-border-default bg-surface-1 flex items-center justify-between gap-2">
          <button
            onClick={() => {
              onClose();
              onStartTask(expert);
            }}
            className="px-5 py-2.5 bg-btn-main text-white font-extrabold text-[13px] rounded-xl flex items-center gap-1.5 shadow-2xs"
          >
            <Play size={15} /> 发起任务
          </button>
          ...
          <div className="flex items-center gap-2 text-[13px] font-extrabold text-text-secondary">
            ...
            <button onClick={() => onAdjustScope(expert)} className="hover:text-text-main px-2 py-1 bg-hover-bg rounded-lg">
              调整授权
            </button>
            <button onClick={() => onEditExpert(expert)} className="hover:text-text-main px-2 py-1 bg-hover-bg rounded-lg">
              编辑专家
            </button>
            <button onClick={() => onToggleStatus(expert)} className="hover:text-danger px-2 py-1 bg-hover-bg rounded-lg">
