...
export const SkillEvalTab: React.FC<SkillEvalTabProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Top Admin Notice */}
      <div className="bg-neutral-900 text-white p-5 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[12px] text-neutral-400 font-bold">
            数据源与网络连接可在“系统设置”内管理
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-neutral-200/90 shadow-xs">
          <span className="text-[11.5px] font-bold text-neutral-400">自动化评测通过率</span>
          <div className="text-[22px] font-black text-neutral-900 mt-1">98.2%</div>
          <span className="text-[11px] font-bold text-emerald-600 mt-0.5 block">回归标准集全部绿色</span>
        </div>

        <div className="bg-white p-4 rounded-2xl border border-neutral-200/90 shadow-xs">
          <span className="text-[11.5px] font-bold text-neutral-400">在线可用契约技能</span>
          <div className="text-[22px] font-black text-neutral-900 mt-1">{skills.filter(s => s.status !== 'needs_config').length} / {skills.length}</div>
          <span className="text-[11px] font-bold text-amber-600 mt-0.5 block">1 项等待配置数据连接</span>
        </div>

        <div className="bg-white p-4 rounded-2xl border border-neutral-200/90 shadow-xs">
          <span className="text-[11.5px] font-bold text-neutral-400">测试集评估综合均分</span>
          <div className="text-[22px] font-black text-neutral-900 mt-1">94.3 分</div>
          <span className="text-[11px] font-bold text-neutral-500 mt-0.5 block">满足通过阈值 ≥ 88 分</span>
        </div>

        <div className="bg-white p-4 rounded-2xl border border-neutral-200/90 shadow-xs">
          <span className="text-[11.5px] font-bold text-neutral-400">本周期自动化调用</span>
          <div className="text-[22px] font-black text-neutral-900 mt-1">2,180 次</div>
          <span className="text-[11px] font-bold text-neutral-500 mt-0.5 block">平均延迟 &lt; 2.4 秒</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-neutral-200/80 shadow-xs overflow-hidden">
        <div className="p-5 border-b border-neutral-100 flex items-center justify-between">
          <h3 className="font-black text-[15px] text-neutral-900 flex items-center gap-2">
        ...
        <div className="divide-y divide-neutral-100">
          {skills.map(skill => {
            ...
            return (
              <div key={skill.id} className="p-5 hover:bg-neutral-50/60 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-extrabold text-[14.5px] text-neutral-900">
                      {skill.name}
                    </span>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {evalInfo && (
                    <div className="text-right">
                      <span className="text-[11px] font-bold text-neutral-400 block">评测测验得分</span>
                  ...
                  <button
                    ...
                    disabled={isRunning}
                    className="px-4 py-2 bg-neutral-900 hover:bg-neutral-800 text-white rounded-xl text-[12px] font-extrabold flex items-center gap-1.5 transition-all shadow-2xs active:scale-95 disabled:opacity-50"
                  >
