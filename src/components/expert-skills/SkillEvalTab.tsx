    ...
    <div className="space-y-6">
      ...
      <div className="bg-btn-main text-white p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 bg-amber-500 text-neutral-950 font-black text-[13px] rounded-full uppercase tracking-wide">
              管理员与高级用户可见
            </span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-surface-1 p-4 rounded-xl border border-border-default/90 shadow-xs">
          <span className="text-[13px] font-bold text-text-tertiary">自动化评测通过率</span>
          <div className="text-[22px] font-black text-text-main mt-1">98.2%</div>
          <span className="text-[13px] font-bold text-emerald-600 mt-0.5 block">回归标准集全部绿色</span>
        </div>

        <div className="bg-surface-1 p-4 rounded-xl border border-border-default/90 shadow-xs">
          <span className="text-[13px] font-bold text-text-tertiary">在线可用契约技能</span>
          <div className="text-[22px] font-black text-text-main mt-1">{skills.filter(s => s.status !== 'needs_config').length} / {skills.length}</div>
          <span className="text-[13px] font-bold text-amber-600 mt-0.5 block">1 项等待配置数据连接</span>
        </div>

        <div className="bg-surface-1 p-4 rounded-xl border border-border-default/90 shadow-xs">
          <span className="text-[13px] font-bold text-text-tertiary">测试集评估综合均分</span>
          <div className="text-[22px] font-black text-text-main mt-1">94.3 分</div>
          <span className="text-[13px] font-bold text-text-tertiary mt-0.5 block">满足通过阈值 ≥ 88 分</span>
        </div>

        <div className="bg-surface-1 p-4 rounded-xl border border-border-default/90 shadow-xs">
          <span className="text-[13px] font-bold text-text-tertiary">本周期自动化调用</span>
          <div className="text-[22px] font-black text-text-main mt-1">2,180 次</div>
          <span className="text-[13px] font-bold text-text-tertiary mt-0.5 block">平均延迟 &lt; 2.4 秒</span>
        </div>
      </div>
              ...
              <div key={skill.id} className="p-5 hover:bg-page-bg transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded text-[13px] font-black bg-hover-bg text-text-secondary">
                      {skill.version}
                    </span>
                    {skill.isComposite && (
                      <span className="px-2 py-0.5 rounded text-[13px] font-black bg-blue-50 text-blue-700 border border-blue-200">
                        复合技能契约
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {evalInfo && (
                    <div className="text-right">
                      <span className="text-[13px] font-bold text-text-tertiary block">评测测验得分</span>
                  ...
                  <button
                    ...
                    disabled={isRunning}
                    className="px-4 py-2 bg-btn-main hover:bg-btn-main-hover text-white rounded-xl text-[13px] font-extrabold flex items-center gap-1.5 transition-all shadow-2xs active:scale-95 disabled:opacity-50"
                  >
