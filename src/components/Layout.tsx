...
import { MessageSquare, Sparkles, X, Terminal, Plus, Gauge, Microscope, LayoutGrid, ClipboardPlus, Workflow, FolderOpen, Brain, Compass, Headset, Moon, Search, Bell, HelpCircle } from 'lucide-react';
      ...
      <aside className="flex flex-col h-full border-r border-zinc-200 p-4 gap-2 bg-zinc-50 w-64 shrink-0 z-50">
        <div className="flex flex-col gap-1 mb-6 px-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-semibold uppercase tracking-widest text-zinc-900">智策系统</h1>
              <p className="text-[10px] text-zinc-400 tracking-wide">v0.9 · 指挥中心</p>
            </div>
          </div>
        </div>
        
        <button className="mb-4 w-full bg-zinc-900 text-white rounded-xl py-2.5 px-4 flex items-center justify-center gap-2 text-[13px] active:scale-95 transition-transform shadow-md shadow-zinc-200">
          <Plus size={14} />
          新建项目
        </button>
        ...
        <header className="flex justify-between items-center w-full px-6 h-14 bg-white/80 backdrop-blur-xl border-b border-zinc-100/50 shadow-sm shadow-zinc-200/50 shrink-0 z-40">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <button className="p-1.5 text-zinc-500 hover:bg-zinc-100 rounded-full transition-colors"><Bell size={20} /></button>
              <button className="p-1.5 text-zinc-500 hover:bg-zinc-100 rounded-full transition-colors"><HelpCircle size={20} /></button>
            </div>
            <div className="w-8 h-8 rounded-full bg-zinc-300 overflow-hidden ring-1 ring-zinc-200 flex items-center justify-center">
              <span className="text-zinc-600 text-xs font-semibold">U</span>
            </div>
          </div>
        </header>
