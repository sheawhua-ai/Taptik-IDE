    ...
    <div className="p-8 max-w-[1400px] mx-auto min-h-full flex flex-col space-y-8">
      <header className="flex justify-between items-end shrink-0">
        <div>
          ...
          <p className="text-text-tertiary mt-2 font-medium">可视化拖拽编排运营流水线，新人一键执行标准化 SOP，并统一审核素材。</p>
        </div>
        <div className="flex gap-3">
          <button className="px-5 py-2.5 bg-surface-1 border border-border-default text-text-secondary rounded-lg text-sm font-bold hover:bg-page-bg transition-colors shadow-sm flex items-center gap-2">
            <Save size={16} />
            存为机构专属 SOP
          </button>
          <button 
            onClick={() => setIsPlaying(!isPlaying)}
            className={`px-5 py-2.5 rounded-lg text-sm font-bold shadow-md transition-colors flex items-center gap-2 ${isPlaying ? 'bg-rose-500 text-white hover:bg-rose-600' : 'bg-btn-main text-white hover:bg-btn-main-hover'}`}
          >
            <Play size={16} />
      ...
      <section className="bg-surface-1 border border-border-default rounded-xl p-8 shadow-sm">
        ...
        <div className="flex items-center gap-2 relative">
          {/* Connecting Line */}
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-hover-bg z-0 rounded-full"></div>
          ...
          {nodes.map((node, index) => (
            <React.Fragment key={node.id}>
              <div className={`relative z-10 flex flex-col items-center gap-3 w-40 cursor-grab active:cursor-grabbing hover:-translate-y-1 transition-transform group`}>
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center shadow-sm border-2 transition-colors ${
                  node.status === 'completed' ? 'bg-emerald-500 border-emerald-500 text-white' :
                  node.status === 'active' ? 'bg-surface-1 border-indigo-500 text-indigo-600 ring-4 ring-indigo-500/20' :
          ...
          <div className="relative z-10 w-14 h-14 rounded-xl bg-page-bg border-2 border-dashed border-neutral-300 text-text-tertiary flex items-center justify-center hover:bg-hover-bg hover:border-neutral-400 cursor-pointer transition-all">
            <Plus size={24} />
          </div>
        </div>
      </section>
      ...
      <section className="bg-btn-main rounded-xl p-1 shadow-lg flex-1 min-h-[400px] flex flex-col">
        ...
        <div className="flex-1 bg-surface-1 rounded-xl m-1 overflow-hidden flex flex-col">
          <div className="p-6 border-b border-border-default flex items-center justify-between">
            <h4 className="text-[16px] font-bold text-text-main">待办审批 (12)</h4>
            <div className="flex gap-2">
              <button className="px-4 py-2 border border-border-default rounded-md text-[13px] font-bold text-text-secondary hover:bg-page-bg">批量打回重拍</button>
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-page-bg grid grid-cols-3 gap-6">
            {[1, 2, 3].map((item) => (
