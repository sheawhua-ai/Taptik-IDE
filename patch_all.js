...
export function ProjectCenter({
  ...
  const renderCreateProjectDrawer = () => {
    return (
      <div className="h-full flex flex-col">
        <div className="p-5 border-b border-neutral-100 flex justify-between items-center bg-white shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="font-bold text-[16px] text-neutral-900">新建项目计划</h3>
            {createStep > 1 && (
              <span className="text-[12px] text-neutral-500 bg-neutral-100 px-2 py-0.5 rounded-full font-medium">第 {createStep} 步 / 4</span>
            )}
          </div>
          <button onClick={() => {
            setDrawerType(null);
        ...
        <div className="flex-1 overflow-y-auto bg-neutral-50/50">
          {createStep === 1 && (
            <div className="p-6 space-y-6 max-w-2xl mx-auto">
              <h4 className="text-[14px] font-bold text-neutral-900 flex items-center gap-2"><Target size={16} className="text-primary-600"/> 第一步：明确运营意图</h4>
              <div className="space-y-4">
                <div>
                  <label className="block text-[13px] font-bold text-neutral-700 mb-1.5">项目名称</label>
                  <input type="text" value={createProjectName} onChange={e=>setCreateProjectName(e.target.value)} placeholder="如：幼犬换粮搜索卡位第三轮" className="w-full border border-neutral-200 rounded-xl px-4 py-2.5 text-[13px] outline-none focus:border-primary-500 shadow-sm" />
                </div>
                <div>
                  <label className="block text-[13px] font-bold text-neutral-700 mb-1.5">本轮希望解决的问题</label>
                  <textarea rows={2} placeholder="如：幼犬肠胃敏感人群转化率低" className="w-full border border-neutral-200 rounded-xl px-4 py-2 text-[13px] outline-none focus:border-primary-500 shadow-sm resize-none"></textarea>
                </div>
                <div>
                  <label className="block text-[13px] font-bold text-neutral-700 mb-1.5">希望观察的结果</label>
                  <textarea rows={2} value={createProjectTarget} onChange={e=>setCreateProjectTarget(e.target.value)} placeholder="如：验证“换粮软便”内容能否提升收藏、有效评论和人工登记的咨询线索" className="w-full border border-neutral-200 rounded-xl px-4 py-2 text-[13px] outline-none focus:border-primary-500 shadow-sm resize-none"></textarea>
                </div>
                <div>
                  <label className="block text-[13px] font-bold text-neutral-700 mb-1.5">约束条件 (可选)</label>
                  <input type="text" placeholder="如：预算5000元，只使用现有账号" className="w-full border border-neutral-200 rounded-xl px-4 py-2 text-[13px] outline-none focus:border-primary-500 shadow-sm" />
                </div>
              </div>
            </div>
          )}
          ...
          {createStep === 2 && (
            <div className="p-6 space-y-6 max-w-2xl mx-auto">
              <h4 className="text-[14px] font-bold text-neutral-900 flex items-center gap-2"><Compass size={16} className="text-primary-600"/> 第二步：检查依据</h4>
              <div className="space-y-4">
                <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm">
                   <div className="text-[13px] font-bold text-neutral-900 mb-3 flex items-center gap-2"><CheckCircle2 size={16} className="text-emerald-500"/> 已具备的事实与经验</div>
                   <ul className="text-[13px] text-neutral-600 list-disc pl-5 space-y-2">
                     <li>历史项目“幼犬换粮第一轮”爆文率为12%</li>
                     <li>操盘手经验：专业开箱风格比图文更容易获客</li>
                   </ul>
                </div>
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 shadow-sm">
                   <div className="text-[13px] font-bold text-amber-900 mb-3 flex items-center gap-2"><AlertTriangle size={16} className="text-amber-600"/> 缺少的关键信息</div>
                   <ul className="text-[13px] text-amber-800 list-disc pl-5 space-y-2 mb-4">
                     <li>近30天竞品痛点评论样本不足</li>
                   </ul>
                   <div className="flex gap-3">
                     <button className="text-[12px] bg-amber-100 text-amber-800 font-bold px-4 py-2 rounded-lg hover:bg-amber-200 transition-colors">补充资料</button>
                     <button className="text-[12px] bg-white border border-amber-200 text-amber-700 font-bold px-4 py-2 rounded-lg hover:bg-amber-50 transition-colors">继续研究</button>
                   </div>
                </div>
                <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm">
                   <div className="text-[13px] font-bold text-neutral-900 mb-2 flex items-center gap-2"><Info size={16} className="text-blue-500"/> 当前只能作为假设的内容</div>
                   <p className="text-[12px] text-neutral-600 pl-6 leading-relaxed">“换粮软便问题目前是目标人群的第一痛点” —— 缺乏最新数据支撑，需在本次执行中重点验证。</p>
                </div>
                <div className="pt-2">
                   <button onClick={() => setCreateStep(3)} className="w-full text-[13px] font-bold text-primary-700 bg-primary-50 hover:bg-primary-100 py-3 rounded-xl transition-colors border border-primary-200">
                     带有假设继续生成草案
                   </button>
                </div>
              </div>
          ...
          {createStep === 3 && (
            <div className="p-6 space-y-6 max-w-2xl mx-auto">
              <div className="flex justify-between items-center mb-2">
                 <h4 className="text-[14px] font-bold text-neutral-900 flex items-center gap-2"><BrainCircuit size={16} className="text-primary-600"/> 第三步：选择策略假设</h4>
                 <button className="text-[12px] font-bold text-primary-600 flex items-center gap-1.5 hover:bg-primary-50 px-3 py-1.5 rounded-lg transition-colors border border-transparent hover:border-primary-100">
                   <MessageSquare size={14} /> 和策略Agent探讨
                 </button>
              </div>
              <div className="space-y-4">
                 <div className="border-2 border-primary-400 bg-primary-50/30 rounded-2xl p-5 shadow-sm cursor-pointer relative overflow-hidden">
                    <div className="absolute top-0 right-0 bg-primary-500 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl">推荐验证</div>
                    <div className="text-[14px] font-bold text-primary-900 mb-4 flex items-center gap-2">
                       <div className="w-6 h-6 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center text-[12px]">A</div>
                       干货科普与信任卡位
                    </div>
                    <div className="space-y-3 text-[13px] bg-white p-4 rounded-xl border border-primary-100">
                      <div className="flex gap-3"><span className="text-primary-700 font-bold shrink-0 w-16">核心假设</span> <span className="text-neutral-700 font-medium">专业育宠师人设可提升咨询转化率</span></div>
                      <div className="flex gap-3"><span className="text-primary-700 font-bold shrink-0 w-16">为何验证</span> <span className="text-neutral-600">操盘手经验表明其优于图文，但缺乏本品牌近期数据验证</span></div>
                      <div className="flex gap-3"><span className="text-primary-700 font-bold shrink-0 w-16">所需资源</span> <span className="text-neutral-600">3个店长号，15篇深度图文</span></div>
                      <div className="flex gap-3"><span className="text-emerald-600 font-bold shrink-0 w-16">判断有效</span> <span className="text-neutral-600">评论区有效咨询占比 &gt; 10%</span></div>
                      <div className="flex gap-3"><span className="text-amber-600 font-bold shrink-0 w-16">主要风险</span> <span className="text-neutral-600">专业内容制作周期较长，可能延误发布节点</span></div>
                    </div>
                 </div>
                 
                 <div className="border border-neutral-200 bg-white hover:border-primary-300 rounded-2xl p-5 shadow-sm cursor-pointer transition-colors group">
                    <div className="text-[14px] font-bold text-neutral-900 group-hover:text-primary-800 mb-4 flex items-center gap-2">
                       <div className="w-6 h-6 rounded-full bg-neutral-100 group-hover:bg-primary-50 group-hover:text-primary-600 flex items-center justify-center text-[12px]">B</div>
                       情绪共鸣与痛点放大
                    </div>
                    <div className="space-y-3 text-[13px] bg-neutral-50 p-4 rounded-xl border border-neutral-100 group-hover:bg-white group-hover:border-primary-50">
                      <div className="flex gap-3"><span className="text-neutral-600 font-bold shrink-0 w-16">核心假设</span> <span className="text-neutral-700 font-medium">侧重养宠焦虑共鸣能获得更高的点击和互动</span></div>
                      <div className="flex gap-3"><span className="text-neutral-600 font-bold shrink-0 w-16">为何验证</span> <span className="text-neutral-600">迎合当前平台焦虑营销趋势</span></div>
                      <div className="flex gap-3"><span className="text-neutral-600 font-bold shrink-0 w-16">所需资源</span> <span className="text-neutral-600">素人号/KOC铺量，30篇开箱图文</span></div>
                      <div className="flex gap-3"><span className="text-neutral-600 font-bold shrink-0 w-16">判断有效</span> <span className="text-neutral-600">整体互动点赞率 &gt; 5%</span></div>
                      <div className="flex gap-3"><span className="text-neutral-600 font-bold shrink-0 w-16">主要风险</span> <span className="text-neutral-600">可能带来高曝光低线索的泛流量</span></div>
                    </div>
                 </div>
                 
                 <div className="pt-4 flex justify-center">
                   <button className="text-[12px] font-medium text-neutral-500 hover:text-neutral-900 underline flex items-center gap-1">
                     <History size={14}/> 参考历史项目生成差异方案
                   </button>
                 </div>
              </div>
            </div>
          ...
          {createStep === 4 && (
            <div className="p-6 space-y-6 max-w-2xl mx-auto">
              <h4 className="text-[14px] font-bold text-neutral-900 flex items-center gap-2"><ListTodo size={16} className="text-primary-600"/> 第四步：生成执行计划</h4>
              
              <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm">
                <div className="text-[13px] font-bold text-neutral-900 mb-4 pb-3 border-b border-neutral-100">首批执行单元</div>
                <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-[12px]">
                  <div><span className="text-neutral-500 block mb-1">目标账号</span> <span className="text-neutral-900 font-medium">全量店长号 (3个)</span></div>
                  <div><span className="text-neutral-500 block mb-1">内容草稿</span> <span className="text-neutral-900 font-medium">自动生成 15 篇</span></div>
                  <div><span className="text-neutral-500 block mb-1">素材要求</span> <span className="text-neutral-900 font-medium">首图需包含痛点文案</span></div>
                  <div><span className="text-neutral-500 block mb-1">发布人与方式</span> <span className="text-neutral-900 font-medium">员工手机人工发布</span></div>
                  <div><span className="text-neutral-500 block mb-1">计划时间</span> <span className="text-neutral-900 font-medium">2026-07-25 前完成首发</span></div>
                  <div><span className="text-neutral-500 block mb-1">数据观察方式</span> <span className="text-neutral-900 font-medium">员工回填链接 + 系统抓取</span></div>
                </div>
              </div>
              
              <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm">
                <div className="flex justify-between items-center mb-4 pb-3 border-b border-neutral-100">
                   <div className="text-[13px] font-bold text-neutral-900">受控工作流步骤</div>
                   <button className="text-[11px] font-bold text-blue-600 flex items-center gap-1 hover:underline">
                     <ShieldCheck size={12}/> 查看执行依据
                   </button>
                </div>
                <div className="space-y-3.5 text-[12px] text-neutral-700">
                  <div className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-[10px] font-bold mt-0.5 border border-blue-100">1</div>
                    <div className="flex-1">检索商家知识与历史项目</div>
                    <div className="text-[10px] bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded font-bold">自动执行</div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-[10px] font-bold mt-0.5 border border-blue-100">2</div>
                    <div className="flex-1">生成选题和内容草稿</div>
                    <div className="text-[10px] bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded font-bold">自动执行</div>
                  </div>
                  <div className="flex items-start gap-3 relative">
                    <div className="absolute left-2.5 top-5 bottom-[-10px] w-px bg-amber-200 border-l border-dashed"></div>
                    <div className="w-5 h-5 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-[10px] font-bold mt-0.5 ring-2 ring-white relative z-10">3</div>
                    <div className="flex-1 font-bold text-amber-900">内容审核</div>
                    <div className="text-[10px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-bold border border-amber-200">需人工确认</div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-[10px] font-bold mt-0.5 border border-blue-100">4</div>
                    <div className="flex-1">匹配现有素材并为缺口创建任务</div>
                    <div className="text-[10px] bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded font-bold">批次确认</div>
                  </div>
                  <div className="flex items-start gap-3 relative">
                    <div className="absolute left-2.5 top-5 bottom-[-10px] w-px bg-amber-200 border-l border-dashed"></div>
                    <div className="w-5 h-5 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-[10px] font-bold mt-0.5 ring-2 ring-white relative z-10">5</div>
                    <div className="flex-1 font-bold text-amber-900">确认账号、发布人和发布任务</div>
                    <div className="text-[10px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-bold border border-amber-200">需人工确认</div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-[10px] font-bold mt-0.5 border border-blue-100">6</div>
                    <div className="flex-1">回填笔记链接并开始观察</div>
                    <div className="text-[10px] bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded font-bold">自动执行</div>
                  </div>
                </div>
              </div>
        ...
        <div className="p-5 border-t border-neutral-100 shrink-0 bg-white flex justify-center shadow-[0_-4px_20px_rgba(0,0,0,0.02)] relative z-10">
          <div className="w-full max-w-2xl flex gap-3">
            {createStep > 1 && (
              <button onClick={() => setCreateStep(s => s - 1)} className="w-[120px] py-3 bg-white border border-neutral-200 text-neutral-700 text-[14px] font-bold rounded-xl hover:bg-neutral-50 transition-colors">上一步</button>
            )}
            {createStep < 4 ? (
              <button onClick={() => setCreateStep(s => s + 1)} className="flex-1 py-3 bg-neutral-900 text-white text-[14px] font-bold rounded-xl hover:bg-neutral-800 transition-colors shadow-sm">下一步</button>
            ) : (
              <button onClick={() => {
                const name = createProjectName || "新营销项目";
                const target = createProjectTarget || "验证“换粮软便”内容能否提升有效评论和咨询线索";
                ...
                setProjects([{
                  ...
                  target: target,
                  stage: "筹备中",
                  blocker: "AI将生成15篇草稿建议；账号分配等将在检查点确认",
                  aiActionCard: null,
                  recommendedAction: "none",
                  pendingCount: 0,
                  period: "未定",
                  aiJudgment: "已根据策略生成计划，正在准备执行单元。",
                  recentChanges: [],
                  batches: []
                } as any, ...projects]);
                ...
                setDrawerType(null);
                setCreateStep(1);
              }} className="flex-1 py-3 bg-neutral-900 text-white text-[14px] font-bold rounded-xl hover:bg-neutral-800 transition-colors shadow-sm">确认策略，创建筹备项目</button>
            )}
          </div>
        </div>
      </div>
